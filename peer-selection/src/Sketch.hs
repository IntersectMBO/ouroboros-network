...
import Data.Graph.Inductive.Graph (LEdge, LNode, LPath (..), labNodes, match, mkGraph)
...
--
-- TODO use one map? Avoids possibility of the same peer being classified in 2
-- ways.
data Peer id = Peer
...
{-
connect :: id -> Edge -> (id -> Endo (Network meta id))
connect a edge = Endo $ \b -> flip Map.adjust a $ \peer -> case edge of
  Cold    -> peer { coldPeers = Map.insert b ColdPeer      (coldPeers peer) }
  Warm dq -> peer { warmPeers = Map.insert b (WarmPeer dq) (warmPeers peer) }
  Hot  dq -> peer { hotPeers  = Map.insert b (HotPeer  dq) (hotPeers  peer) }
-}
...
all_pairs_sp
  ...
  -> Network meta Int
  -> Map (Int, Int) m
all_pairs_sp edgeWeight network = Map.fromList sps

  where

  graph :: Gr (Node meta) m
  graph = second edgeWeight (toGraph id network)

  sps :: [((Int, Int), m)]
  sps = [ ((a, b), weight) | (a, _) <- labNodes graph, (b, weight) <- unroll_tree (sps_at a) ]

  -- For each identifier, make the shortest path tree.
  sps_at :: Int -> LRTree m
  sps_at id = spTree id graph

  unroll_tree :: LRTree m -> [(Int, m)]
  unroll_tree []             = []
  unroll_tree (lpath:lpaths) = unLPath lpath ++ unroll_tree lpaths

newtype Hops = Hops
  { getHops :: Word64 }

deriving instance Eq Hops
deriving instance Ord Hops
deriving instance Show Hops

instance Semigroup Hops where
  a <> b = Hops (getHops a + getHops b)
...
{-
-- Only for hot edges with non-bottom delta Q...
-- Will this even be interesting though? If the Delta Q is uniform over all
-- nodes then this will be essentially the same as the number of hops.
hops :: Edge -> Hops
hops (Hot Bottom
-}

-- What if the graph is not completely connected?
--characteristic_path_length :: Map (Int, Int) Hops -> Rational
--characteristic_path_length = median . mean . Map.elems

-- | A latency monoid: the G and S delta Q for a given number of bytes.
data Latency where
  Delay :: !DiffTime -> Latency
  Loss  :: Latency

deriving instance Eq Latency

instance Ord Latency where
  Loss    `compare` Loss    = EQ
  Loss    `compare` _       = GT
  _       `compare` Loss    = LT
  Delay a `compare` Delay b = a `compare` b
...
-- | Latency at an edge for a given number of bytes to transmit.
-- Only hot edges give non-loss, because cold and warm edges are never used
-- to transmit.
latency_at :: Natural -> Edge -> Latency
latency_at _ Cold               = Loss
latency_at _ (Warm _)           = Loss
latency_at _ (Hot Bottom)       = Loss
latency_at n (Hot (DeltaQ g s)) = Delay (g + s n)
