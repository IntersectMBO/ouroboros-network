...
-- >
--
newtype Versions vNum extra r agreedOptions  = Versions
  { getVersions :: Map vNum (Sigma (Version extra r agreedOptions))
  }
  deriving (Semigroup)

{-instance Functor (Versions vNum extra agreedOptions) where
...
foldMapVersions :: (Ord vNum, Foldable f, HasCallStack)
                => (x -> Versions vNum extra r a)
                -> f x
                -> Versions vNum extra r a
...
class Acceptable vn v agreedOptions where
  acceptableVersion :: vn -> v -> v -> Accept agreedOptions
...
data DictVersion vNumber agreedOptions vData where
     DictVersion :: ( Typeable vData
                    , Acceptable vNumber vData agreedOptions
                    , Show vData
                    )
