...
import           Network.Mux.Types (MuxBearer (..), MuxSDU (..),
                   RemoteClockModel (..), setTimestamp)
import           Network.Mux.Trace (MuxTrace (..))

import           Ouroboros.Network.ConnectionId
import           Ouroboros.Network.IOSim.BufferedChannel
...
-- todo: this should be defined in `network-mux`.
type SDUSize = Word16
...
data FD_ m addr
    ...
    | FDListening
        ...
        -- ^ listening queue; when 'connect' is called; dual 'BufferedChannel'
...
data SnocketTrace m addr
    ...
    | STClosedWhenReading
    | STBearer       (FD_ m addr)
    | STBufferedChannelTrace (ConnectionId addr) BufferedChannelTrace
  deriving Show
...
mkSnocket state tr = Snocket { getLocalAddr
  ...
  where
                    ...
                    FDListening remoteAddress' queue ->
                      ...
                      (chan, chan')  <-
                        newConnectedBufferedChannelPair
                          (STBufferedChannelTrace ConnectionId { localAddress,
                                                                 remoteAddress
                                                               }
                           `contramap` tr)
                          (STBufferedChannelTrace ConnectionId { localAddress = remoteAddress,
    ...
    toBearer :: DiffTime
             ...
             -> m (MuxBearer m)
    toBearer _sduTimeout muxTracer FD { fdVar } = do
        fd <- atomically (readTVar fdVar)
        case fd of
          FDUninitialised {} ->
          ...
          FDConnected _ chan sduSize -> do
            ...
            return $ mkBearer chan sduSize
          FDClosed {} ->
            throwIO invalidError
      where
        mkBearer :: BufferedChannel m
                 -> SDUSize
                 -> MuxBearer m
        mkBearer  chan sduSize = 
            MuxBearer {
              read    = readMux,
              write   = writeMux,
              sduSize
