...
withBidirectionalConnectionManager name timeouts
    ...
    withConnectionManager
      ConnectionManagerArguments {
          ...
          cmStdGen,
          cmConnectionsLimits = acceptedConnLimit,
          cmUpdateVersionData = \versionData diffusionMode ->
          ...
          $ \_ serverAddr _serverAsync -> do
            ...
            (rs :: [Either SomeException (TemperatureBundle [resp])]) <-
                replicateM
                  (numberOfRounds clientAndServerData)
                  (bracket
                     (requestOutboundConnection connectionManager InitiatorOnlyDiffusionMode serverAddr)
                     (\case
                        Connected connId _ _ -> unregisterOutboundConnection connectionManager connId
                    ...
                    (\case
                      Connected connId _ _ ->
                        unregisterOutboundConnection
                          ...
                          connId
                      Disconnected {} ->
                        error "ibidirectionalExperiment: mpossible happened")
                    (\connHandle ->
                      case connHandle of
                        Connected connId _ (Handle mux muxBundle controlBundle _) -> do
                          try @_ @SomeException $
