...
import           Data.Foldable (fold, foldl', traverse_, forM_)
...
genScheduleOutbound size = do
    ...
    seActiveDelay <- resize (ceiling maxActive) (genDelayWithPrecision precision)
...
genScheduleInbound size = do
    ...
    seStart <-
      frequency
        -- [ (0,  return 0)
          -- how to lift the assumption (> 0)!
          -- when 'seStart == 0' for an inbound connection we
          -- simulate the race condition between outbound and
          -- inbound threads.  In such case the outbound side
          -- must error when calling 'connect'.
    ...
    maxActive <- resize size (genDelayWithPrecision precision)
                 `suchThat` (> 1)
    seHandshakeDelay
      <- suchThat (resize 2 (genDelayWithPrecision precision))
                  (   (< (maxActive / 3))
                   /\ (> 0)
                  )
    seActiveDelay <- resize (round maxActive) (genDelayWithPrecision precision)
                    `suchThat` (\a -> a > 0 && a + seHandshakeDelay <= maxActive)
    ...
    let size' = size `div` 5
    seRemoteTransitions <-
          fixupRemoteTransitions seActiveDelay
    ...
    shrink = go
      where
        ...
        go (sa@ScheduleOutbound { seStart
            ...
            [ sa { seStart = seStart' }
            | Delay seStart' <- shrink (Delay seStart)
            , seStart' > 0
            ]
          ++
            [ sa { seConnDelay = bimap getDelay getDelay seConnDelay' }
            | seConnDelay' <- shrink (bimap Delay Delay seConnDelay)
            , either getDelay getDelay seConnDelay' > 0
            ]
          ++
            [ sa { seActiveDelay = seActiveDelay'
                 }
            | Delay seActiveDelay' <- shrink (Delay seActiveDelay)
            ]
          ++
            [ sa { seHandshakeDelay = seHandshakeDelay' }
            | seHandshakeDelay' <- getDelay `map` shrink (Delay seHandshakeDelay)
            ]
          ++
        ...
        go (sa@ScheduleInbound { seStart
          ...
          ++
            ...
            | seHandshakeDelay' <-
                getDelay `map` shrink (Delay seHandshakeDelay)
            ]
          ++
    ...
    entryLifeTime =
      case s of
        ...
        ScheduleOutbound { seConnDelay = Right connDelay, seHandshakeDelay } ->
            connDelay + seHandshakeDelay + seActiveDelay s
        ScheduleInbound { seHandshakeDelay } ->
            seHandshakeDelay + seActiveDelay s
...
data State = State {
    ...
    , outboundUntil  :: !(Maybe (Time, Time, Bool))

      -- | Time when inbound connection will terminate.
    , inboundUntil   :: !(Maybe Time)
    }
  deriving Show
...
fixupSchedule =
  ...
  where
    ...
    updateState time df State { dataFlow
                              , handshakeUntil
                              , outboundUntil
                              , inboundUntil
                              } =
      let -- Note: using '<' instead of '<=' is matter when connections are
          -- considered terminated.  With '<' the @fixupSchedule'@ is
          -- nilpotent, which is a nice property to have for a fixup function. 
          outboundUntil' =
            (case outboundUntil of
              Just (_, a, _) | a < time -> Nothing
              _                         -> outboundUntil)
          inboundUntil' =
          ...
          dataFlow' = if   isJust inboundUntil'
                        || isJust outboundUntil'
                        then dataFlow
                        else Just df
      in State { time
               , dataFlow       = dataFlow'
    ...
    initialState = State
            ...
            , outboundUntil  = Nothing
            , inboundUntil   = Nothing
            }
    ...
    go !s !acc (a@ScheduleOutbound
      ...
      let t  = seStart `addTime` time s
          ...
          -- first need to block until the TerminatingState is passes)
          --
          ...
          --
          t' = case       ((\(_, b, e) -> (b, e    )) <$> outboundUntil s)
                    `max` ((\    b     -> (b, False)) <$> inboundUntil s) of
                Nothing              -> t
                -- the previous connection did not errored, we need to add 'testTimeWaitTimeout'
                Just (until_, False) -> t `max` (testTimeWaitTimeout `addTime` until_)
                -- the previous connection errored, no need to add 'testTimeWaitTimeout'
                Just (_, True)       -> t
          hasOutbound' = hasOutbound s'
          hasInbound'  = hasInbound  s'
      -- @exists || reused@ is true only if 'includeOutboundConnection' will
      -- call 'connect'.
      in case seConnDelay of
      -- in Debug.traceShow s' $ case seConnDelay of
        -- no outbound nor inbound connection; 'connect' fails
        (Left connDelay) | not hasOutbound'
                         , not hasInbound' ->
          let outboundUntil' = Just ( t
                                    , connDelay `addTime` t'
                                    , True
                                    )
              si = ScheduleInfo {
        ...
        (Right connDelay) | not hasOutbound'
          ...
          let handshakeUntil' = seHandshakeDelay `addTime` connDelay `addTime` t'
              outboundUntil'  = ( t
                                , seActiveDelay `addTime` handshakeUntil'
                                , False
                                )

              s'' = s' { outboundUntil  = Just outboundUntil'
        ...
        _ | Just until <- inboundUntil s' ->
          let si = ScheduleInfo {
              ...
              s'' = s' { outboundUntil =
                           if siForbidden si
                             then outboundUntil s'
                             else Just ( t
                                       , seActiveDelay
                                         `addTime`
                                         -- The outbound connection will start
                                         -- when ongoing handshake will finish.
                                         maybe t (`max` t) (handshakeUntil s')
                                       , isLeft seConnDelay 
                                       )
                       }

              thisUntil = (if siForbidden si
                             then 0
                             else seActiveDelay
                          )
                          `addTime`
              ...
              a' = a { seDataFlow = 
                          case dataFlow s' of
                            ...
                            Nothing -> error "fixupSchedule: invariant violation"
                      , seConnDelay = Right 0
                      , seHandshakeDelay = maybe 0 (\d -> (d `diffTime` t) `max` 0) (handshakeUntil s)
                      , seExtra = si
                      }
          in if thisUntil >= until
             then go s              acc  as
             else go s'' (a' : acc) as

        _ | otherwise -> error "Hello captain, you landed in uncharted teritory!" -- impossible happend ;)

    go !s !acc (a@ScheduleInbound { seStart
                                  , seHandshakeDelay
                                  , seDataFlow
                                  , seActiveDelay
                                  } : as) =
      let t  = seStart `addTime` time s
          ...
          hasOutbound' = case outboundUntil s' of
                           ...
                           -- transition.
                           Just (started, _until, _err) -> started < t
          hasInbound'  = hasInbound s'
      in if hasInbound' || hasOutbound'
         ...
         then go s acc as
           ...
           let handshakeUntil' = seHandshakeDelay `addTime` t
               time' = seActiveDelay `addTime` handshakeUntil'
               siOverwrite =
...
prop_fixupSchedule (Schedule schedule) =
    ...
    fixupSchedule schedule == schedule
    .&&. 
...
validRefinedSchedule (Schedule s) =
  ...
  where
    validScheduleInfo se =
        ...
        && (siExists    `implies` isLeft (seConnDelay se))
...
prop_generator_RefinedSchedule a@(Schedule schedule) =
    ...
    (label $ concat
           [ "connection error "
           , if outNo > 0
               then mkLabel 5 ((100 * length ( filter (   not . siExists . seExtra
                                                       /\ not . siReused . seExtra
                                                       /\ isLeft . seConnDelay)
                                                -- 'seConnDelay' is partial function!
    ...
    (label $ concat
           [ "exists "
           , if outNo > 0
               then mkLabel 5 ((100 * length (filter (siExists . seExtra)
                                                     schedule))
                               `div` outNo
                              )
               else "0"
...
-- | Connection handle. Gives access to 'RefinedScheduleEntry'.
--
...
mkConnectionHandler snocket =
    ConnectionHandler $
      WithInitiatorResponderMode
        ...
        handler
  where
    handler :: ConnectionHandlerFn handlerTrace (FD m) Addr Handle Void Version m
...
--
-- Execute the 'RefinedScheduleMap'.
--
prop_connectionManagerSimulation
    ...
    -- way.
    -> Property
prop_connectionManagerSimulation bindToLocalAddress scheduleMap =
    -- Debug.trace ("START\n" ++ prettyScheduleMap scheduleMap) $
  ...
  where
    ...
    verifyTrace = conjoin
      ...
      where
        ...
        verifyTransitionProp tr = counterexample (show tr) (verifyAbstractTransition tr)
                      ...
                      thread <-
                        asyncWithUnmask $ \unmask -> unmask $ do
                          ...
                          case r of
                            ...
                            Right _ -> do
                              threadDelay (seActiveDelay conn)
                              ...
                              _ <- 
                                unregisterOutboundConnection
                                  connectionManager addr
                                `concurrently`
                                  (unless (siReused (seExtra conn) && seDataFlow conn == Duplex) $
                                     void $ unregisterInboundConnection connectionManager addr
                                  )
                              return ()
                        ...
                        (Accepted fd' addr', acceptNext) -> do
                          thread <-
                            async $ do
                              ...
                              let waitUntil = seActiveDelay conn `addTime` t
                              t' <- getMonotonicTime
                              unless (t' <= waitUntil)
                                   (throwIO (EventTimingError
                                              (concat
