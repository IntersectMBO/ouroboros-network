...
the client's responsibility not to reconnect too early. For example, after
a failed connection attempt \ptopgov{} delays reconnecting to that peer using
an exponential back-off: \texttt{5}s, \texttt{10}s, \texttt{20}s, \texttt{40}s,
\texttt{80}s and \texttt{160}s, each fuzzed by up to \texttt{2}s in either
direction.  A peer which was demoted because one of its mini-protocols errored
is not re-promoted for a configured delay, for which
\texttt{ouroboros-network} provides \texttt{10}s as the default.
