...
\begin{figure}[h]
  \begin{tabular}{|l|l|}
    ...
    Client has Agency & \MsgRequestTxIdsNB, \MsgRequestTxIdsB, \StTxIdsNonBlocking, \StTxs \\\hline
  \end{tabular}
\end{figure}
...
\begin{itemize}
  \item chain-sync mini-protocol for headers (section~\ref{chain-sync-protocol}
  \item block fetch mini-protocol (section~\ref{block-fetch-protocol})
...
\begin{itemize}
  \item chains synchronisation mini-protocols for blocks (section~\ref{chain-sync-protocol})
  \item local transaction submission mini-protocol (section~\ref{local-tx-submission-protocol})
  \item local state query mini-protocol; from version $NodeToClient\_2$ (section~\ref{local-state-query-protocol})
\end{itemize}
