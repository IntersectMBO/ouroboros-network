...
remark applies to node-to-client protocol as well.} to a record which consists
of
\begin{description}
...
version data associated to the negotiated version number.  The result of
negotiation is a new version data record which consists of:
\begin{itemize}\label{alg:node-to-node-negotiation}
  \item if the network magic agree then its inherited by the negotiated version
    data, otherwise the negotiation fails;
  \item diffusion mode SHOULD be initiator only iff any side proposes the
  ...
  \item query SHOULD be inherited from the client (the side that sent
      \MsgProposeVersions{}).
\end{itemize}
If negotiation was successful, the negotiated version data is sent back
...
The negotiated version data is computed similar as in the node-to-node
protocol:
...
other side as termination message \MsgReplyVersions{}, the same will
...
be split into multiple segments.  The Handshake protocol is using the
mini-protocol nuumber $0$ in both node-to-node and node-to-client case.
