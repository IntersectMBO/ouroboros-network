...
\noindent\href{https://hackage.haskell.org/package/typed-protocols}{\emph{typed-protocols documentation}}
...
\miniEntry
    ...
    {typed-protocols/examples/Network/TypedProtocol/PingPong/Type.hs}
    {https://intersectmbo.github.io/typed-protocols/typed-protocols/examples/Network-TypedProtocol-PingPong-Type.html\#t:PingPong}
...
\miniEntry
    ...
    {https://intersectmbo.github.io/typed-protocols/typed-protocols/examples/Network-TypedProtocol-ReqResp-Type.html\#t:ReqResp}
...
\miniEntry
    {Object-Diffusion Protocol}
    {object-diffusion-protocol}
    {A generic mini-protocol for diffusing objects; instantiated for Peras certificates and votes}
    {ouroboros-network/protocols/lib/Ouroboros/Network/Protocol/ObjectDiffusion/Type.hs}
    {https://ouroboros-network.cardano.intersectmbo.org/ouroboros-network/protocols/Ouroboros-Network-Protocol-ObjectDiffusion-Type.html\#t:ObjectDiffusion}
...
\haddockrefraw{Network.TypedProtocol.PingPong.Type}{https://intersectmbo.github.io/typed-protocols/typed-protocols/examples/Network-TypedProtocol-PingPong-Type.html\#t:PingPong}
...
\haddockrefraw{Network.TypedProtocol.ReqResp.Type}{https://intersectmbo.github.io/typed-protocols/typed-protocols/examples/Network-TypedProtocol-ReqResp-Type.html\#t:ReqResp}
...
\codechaddockref{Cardano.Network.Protocol.Handshake.Codec}{cardano-diffusion/protocols/Cardano-Network-Protocol-Handshake-Codec.html\#v:nodeToNodeHandshakeCodec}\\
\codechaddockref{Cardano.Network.Protocol.Handshake.Codec}{cardano-diffusion/protocols/Cardano-Network-Protocol-Handshake-Codec.html\#v:nodeToClientHandshakeCodec}\\
...
\newcommand{\MsgReplyVersions}{\msg{MsgReplyVersions}}
\newcommand{\MsgAcceptVersion}{\msg{MsgAcceptVersion}}
\newcommand{\MsgQueryReply}{\msg{MsgQueryReply}}
...
\begin{figure}[h]
  \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=6cm, semithick]
    ...
    \draw[->] (StConfirm.30)  to [out=35,  in=145] node[above]{\MsgAcceptVersion} (StDone.150);
    \draw[->] (StConfirm.10)  to [out=15,  in=165] node[above]{\MsgQueryReply}    (StDone.170);
...
\begin{description}
  ...
  \item [\MsgQueryReply{} {\boldmath $(versionTable)$}]
      The server replies with all the versions it supports together with their
      version data, and the connection is terminated.  This message is sent
      instead of \MsgAcceptVersion{} whenever the negotiated version data
      requests a version query (see the \texttt{query} flag of the
      node-to-node and node-to-client version data below).  No mini-protocols
      are run in this case.
...
\begin{table}[h]
  \begin{tabular}{l|l|l|l}
    ...
    \StConfirm & \MsgQueryReply      & $versionTable$                & \StDone \\
...
\begin{description}
  ...
  \item[peer sharing] either $0$ or $1$: $1$ indicates that the node is
    willing to engage in peer sharing.  Note that this is the value
    \emph{proposed} by a node; the PeerSharing mini-protocol is run only if
    the \emph{negotiated} value is $1$;
  \item[query] a boolean value: \texttt{True} asks the remote side to send
    back all supported versions \& version data and terminate the connection;
  \item[peras support] a boolean value: \texttt{True} indicates that the node
    supports the Peras mini-protocols.  As for peer sharing, this is the value
    \emph{proposed} by a node; the Peras mini-protocols are run only if the
    \emph{negotiated} value is \texttt{True}.  This field is only present from
    \texttt{NodeToNodeV\_16} on; for earlier versions it MUST NOT be encoded
    and it is negotiated as \texttt{False}.
\end{description}
...
\begin{itemize}\label{alg:node-to-node-negotiation}
  ...
  \item peer sharing SHOULD be enabled (i.e. $1$) if and only if both sides
    propose it enabled (i.e. the logical conjunction operator);
  \item peras support SHOULD be \texttt{True} if and only if both sides propose
    \texttt{True} (i.e. the logical conjunction operator);
  \item query SHOULD be \texttt{True} if and only if any side proposes
    \texttt{True} (i.e. the logical disjunction operator).
\end{itemize}
Note that both mini-protocol feature flags -- peer sharing and peras support
-- are combined with conjunction.  This is necessary because the negotiated
value decides which mini-protocols are run by the multiplexer on both ends of
the connection, and hence both ends MUST arrive at the same value: a node
which does not support a feature will not run the corresponding mini-protocol,
not even its responder side.

The rules above are \emph{symmetric}, i.e. they do not depend on which side is
the client and which one is the server.  This is required to support TCP
simultaneous open, in which case both sides compute the negotiated version
data independently, see section~\ref{sec:handshake-implementation}.  They are
also \emph{idempotent} in the following sense: applying them to the local
version data and an already negotiated version data gives back the negotiated
version data.  This is required because the client verifies the version data
received with \MsgAcceptVersion{} by re-running the negotiation against its
own local version data.

If the negotiated query flag is \texttt{True}, the server MUST reply with
\MsgQueryReply{}, see section~\ref{sec:handshake-implementation}.  Otherwise,
if the negotiation is successful, the negotiated version data is sent back
using \MsgAcceptVersion{}, and if it fails \MsgRefuse{} SHOULD be sent.
...
The node-to-client handshake instantiates version data to a record which consists
of
\begin{description}
  \item[network magic] a \texttt{Word32} value;
  \item[query] a boolean value: \texttt{True} asks the remote side to send
...
\label{sec:handshake-implementation}
...
server replies with an \MsgAcceptVersion{} message containing the negotiated
version number and version data, with a \MsgQueryReply{} message containing
all the versions it supports (if the negotiated query flag is set), or with
a \MsgRefuse{} message.
...
\item
  Otherwise, compute the negotiated version data according to the
    algorithm~\ref{alg:node-to-node-negotiation} or
    \ref{alg:node-to-client-negotiation}.
\item
  If the query flag of the negotiated version data is set:
  Reply with \MsgQueryReply{} and the version table containing all the
  versions the server supports together with their version data, and terminate
  the connection.  No mini-protocols are run in this case.
\item
  Otherwise, encode the negotiated version data with the corresponding CBOR
    codec and reply with \MsgAcceptVersion, the selected version number
    and the negotiated version data.
\end{enumerate}
Note that in step 4), 6), 8) and 10) the handshake protocol uses the callback
functions that are specific for a set of protocols that the server supports.
...
When the client receives \MsgAcceptVersion{}, it MUST verify the server's
choice: it MUST reject a version number which it did not propose, and it MUST
check that the received version data is acceptable to it.  This check SHOULD
be done by re-running the negotiation algorithm on the client's own local
version data and the received version data; its result is then the version
data the client uses.  If the check fails, the client MUST abort the
handshake.  Since the negotiation algorithm is idempotent (see
algorithm~\ref{alg:node-to-node-negotiation}
and~\ref{alg:node-to-client-negotiation}) a well behaved server will always
pass this check.
...
property, see
\href{https://github.com/intersectmbo/ouroboros-network/blob/master/ouroboros-network/protocols/tests-lib/Ouroboros/Network/Protocol/Handshake/Test.hs}{Ouroboros-Network
test suite}.
...
(\href{https://ouroboros-network.cardano.intersectmbo.org/ouroboros-network/protocols-tests-lib/Ouroboros-Network-Protocol-Handshake-Test.html\#v:prop\_channel\_simultaneous\_open}{\texttt{prop\_channel\_simultaneous\_open}})
...
The version data is specified in two separate listings, imported by the
handshake specification under the \texttt{v14} and \texttt{v16} namespaces:
\texttt{NodeToNodeV\_14} and \texttt{NodeToNodeV\_15} use the former,
\texttt{NodeToNodeV\_16} the latter (which adds the \texttt{perasSupport}
field).
\lstinputlisting[style=cddl]{../../cardano-diffusion/protocols/cddl/specs/handshake-node-to-node-v14.cddl}
\lstinputlisting[style=cddl]{../../cardano-diffusion/protocols/cddl/specs/node-to-node-version-data-v14.cddl}
\lstinputlisting[style=cddl]{../../cardano-diffusion/protocols/cddl/specs/node-to-node-version-data-v16.cddl}
...
The timeout on state \StIdle{} is configurable and it can be disabled
altogether; the value given below is the default used by Cardano Node.
The timeout on state \StMustReply{} only applies if the peer is not trustable. Trusted peers are assumed to be honest and will send new headers as soon as they receive them. If a trusted peer has not sent new headers for an extended period, it is assumed that no new headers are available. Typically, trusted peers are operated by the same organisation, such as all nodes managed by a single SPO.
...
Note that by sending \MsgIntersectNotFound{}, the server will not modify its
\readpointer{}.
...
\codechaddockref{Cardano.Network.Protocol.PeerSharing.Codec}{cardano-diffusion/protocols/Cardano-Network-Protocol-PeerSharing-Codec.html\#v:codecPeerSharing}\\
...
Whether a node participates in peer sharing is a configuration choice.  The
configured value is the one \emph{proposed} in the handshake version data, and
the mini-protocol is run only if the \emph{negotiated} value is $1$, i.e. only
if both sides propose it enabled, see
algorithm~\ref{alg:node-to-node-negotiation}.

A relay node SHOULD enable peer sharing; a block producer SHOULD NOT.  These
are the defaults provided by
\href{https://ouroboros-network.cardano.intersectmbo.org/ouroboros-network/Ouroboros-Network-Diffusion-Configuration.html\#v:defaultPeerSharing}{\texttt{defaultPeerSharing}}.
The distinction matters because the flag governs more than whether this
mini-protocol is run:
\begin{itemize}
  \item a node MUST NOT share the address of a peer which proposed peer
    sharing disabled.  Proposing peer sharing disabled is therefore how a node
    opts out of having its own address diffused, which is the reason a block
    producer disables it;
  \item a node which is configured with peer sharing disabled does not request
    peers over this mini-protocol, and it also does not learn addresses from
...
\item [\MsgSharePeers{} ${[}peerAddress{]}$]
  ...
  successful inbound or outbound session, and which have not failed since.

  A $peerAddress$ is a resolved socket address: an \texttt{IPv4} or
  \texttt{IPv6} address together with a port number, see
  section~\ref{peersharing-cddl} for the encoding.  The mini-protocol carries
  neither DNS names nor \texttt{SRV} records
  (\href{https://cips.cardano.org/cip/CIP-0155}{CIP-0155}); a peer which is
  configured with a domain name, e.g. in a topology file, is shared as
  a resolved socket address, never by its name.  The encoding of
  $peerAddress$ is the same for all currently supported node-to-node
  versions.
\item [\MsgDone]
  Terminating message.
\end{description}
...
\protocolhaddockref{Ouroboros.Network.Protocol.ObjectDiffusion.Type}{ouroboros-network/protocols/Ouroboros-Network-Protocol-ObjectDiffusion-Type.html\#t:ObjectDiffusion}\\
\codechaddockref{Ouroboros.Network.Protocol.ObjectDiffusion.Codec}{ouroboros-network/protocols/Ouroboros-Network-Protocol-ObjectDiffusion-Codec.html\#v:codecObjectDiffusion}\\
\hyperref[table:node-to-node-protocol-numbers]{\textit{node-to-node mini-protocol numbers}}: \texttt{16}, \texttt{17}\\
\label{object-diffusion-protocol}
\label{peras-protocol}

\subsection{Description}

The \textit{object-diffusion mini-protocol} diffuses a stream of objects of
a single kind from one node to another.  It is generic: it is parametrised by
the type of the objects and by the type of their identifiers, and the encoding
of $objectId$ and $object$ is supplied by the instantiation (see
section~\ref{object-diffusion-cddl}).

The node-to-node protocol instantiates it twice, for the diffusion of Peras
certificates and of Peras votes.  Both instances are run only if \textit{peras
support} was negotiated, which requires both sides to propose it (see
algorithm~\ref{alg:node-to-node-negotiation}), hence they are available from
\texttt{NodeToNodeV\_16} onwards.  The two instantiations differ only in the
mini-protocol number, the object type and the limits given in
table~\ref{table:peras-instantiations}.

The protocol is pull-based, like \textit{chain-sync} and \textit{block-fetch}
and unlike \textit{tx-submission}: the client is the side which
\textit{receives} objects and it drives the exchange, while the server sends
objects when asked to.  Objects are transferred in two steps: the client first
asks for object identifiers and then asks for those objects it is interested
in.  This makes the protocol usable in a trustless setting, where each side has
to guard against resource consumption attacks from the other.

The protocol maintains a notional FIFO queue of \textit{outstanding} object
identifiers, which the server has announced but the client has not acknowledged
yet.  Only outstanding objects can be requested; they can be requested in any
order, but at most once.  Identifiers are acknowledged in the FIFO order in
which they were announced, and the acknowledgement is piggybacked on the
message which asks for more identifiers.  It is a protocol error to put the
number of outstanding identifiers over the limit given in
table~\ref{table:peras-instantiations}.

\newcommand{\StObjectIdsB}      {\state{StObjectIdsBlocking}}
\newcommand{\StObjectIdsNB}     {\state{StObjectIdsNonBlocking}}
\newcommand{\StObjects}         {\state{StObjects}}
\newcommand{\MsgRequestObjIdsB} {\msg{MsgRequestObjectIdsBlocking}}
\newcommand{\MsgRequestObjIdsNB}{\msg{MsgRequestObjectIdsNonBlocking}}
\newcommand{\MsgReplyObjectIds} {\msg{MsgReplyObjectIds}}
\newcommand{\MsgRequestObjects} {\msg{MsgRequestObjects}}
\newcommand{\MsgReplyObjects}   {\msg{MsgReplyObjects}}
...
\begin{tikzpicture}[->,shorten >=1pt,auto,node distance=4.5cm, semithick]
  ...
  \node[state]                   (B) at ( 8,  0) {\StDone};
  \node[state, myblue]           (C) at ( 4, -4) {\StObjectIdsB};
  \node[state, myblue]           (D) at (-4, -4) {\StObjectIdsNB};
  \node[state, myblue]           (E) at ( 0,  4) {\StObjects};
  \draw (I) edge[above]               node[above]{\MsgInit}                                                   (A);
  \draw (A) edge[above]               node[above]{\MsgDone}                                                   (B);
  \draw (A) edge[left, bend left=45]  node[fill = white, anchor = center]{\MsgRequestObjIdsB}                 (C);
  \draw (C) edge[right, bend left=15] node[fill = white, anchor = center, above = 2pt]{\MsgReplyObjectIds}    (A);
  \draw (D) edge[right, bend left=45] node[fill = white, anchor = center]{\MsgReplyObjectIds}                 (A);
  \draw (A) edge[right, bend left=15] node[fill = white, anchor = center, below = 2pt]{\MsgRequestObjIdsNB}   (D);
  \draw (A) edge[left, bend right=45] node[fill = white, anchor = center, above = 2pt]{\MsgRequestObjects}    (E);
  \draw (E) edge[right, bend right=45]node[fill = white, anchor = center, below = 2pt]{\MsgReplyObjects}      (A);
\end{tikzpicture}
  \caption{State machine of the Object-Diffusion mini-protocol.}
  \label{object-diffusion-automata}
\end{figure}
...
\begin{description}
\item [\MsgInit]
  Initial message of the protocol.  It carries no payload and, unlike in
  \textit{tx-submission}, it does not transfer agency: the next message is sent
  by the client as well.  It is kept so that the mini-protocol starts with
  a client message, and to leave room for a future payload which would let the
  server resume from the last point known to the client.
\item [\MsgRequestObjIdsB{} {\boldmath $(ack,req)$}]
  The client acknowledges $ack$ outstanding object identifiers and asks for up
  to $req$ new ones; $req$ MUST be greater than zero.

  This is a blocking operation: the reply will contain at least one identifier
  and there is no timeout.  It covers the case when there is nothing else to do
  but wait.  This variant MUST be used if and only if no unacknowledged
  identifiers remain after taking $ack$ into account.
\item [\MsgRequestObjIdsNB{} {\boldmath $(ack,req)$}]
  As above, but a non-blocking operation: the reply may be empty and a prompt
  response is expected.  It covers the high throughput case, in which requests
  for identifiers are interleaved with requests for objects and hence must not
  block.  Either $ack$ or $req$ MUST be non-zero.  This variant MUST be used if
  and only if unacknowledged identifiers remain after taking $ack$ into
  account.

  Both variants are encoded as a single message, distinguished by a boolean
  flag; see section~\ref{object-diffusion-cddl}.
\item [\MsgReplyObjectIds{} {\boldmath ($\langle objectId \rangle$)}]
  The server replies with the identifiers of available objects, in the order in
  which it wishes to submit them.  The list MUST NOT be longer than $req$; it
  MUST be non-empty in the blocking case and MAY be empty in the non-blocking
  case.  The identifiers are appended to the FIFO of outstanding identifiers.
\item [\MsgRequestObjects{} {\boldmath ($\langle objectId \rangle$)}]
  The client requests objects by their identifiers.  It is a protocol error to
  ask for an identifier which was not announced by \MsgReplyObjectIds{}, which
  is no longer outstanding, or which was already asked for.  While the server
  is responsible for staying within the in-flight limits, the client must
  cooperate by keeping the total requested across all in-flight requests within
  them as well.
\item [\MsgReplyObjects{} {\boldmath ($\langle object \rangle$)}]
  The server replies with the requested objects.  An object may have become
  invalid between being announced and being requested, hence the reply MAY omit
  objects; omitted objects are to be treated as if the server had never
  announced them, which is no guarantee that they are invalid --- they may
  still be available from another peer.
\item [\MsgDone]
  Terminating message, sent by the client when idle.
\end{description}
...
\begin{table}[h!]
  ...
  \begin{tabular}{l|l|l|l}
    \header{from state} & \header{message}      & \header{parameters}            & \header{to state} \\\hline
    \StInit             & \MsgInit              &                                & \StIdle           \\
    \StIdle             & \MsgRequestObjIdsB    & $ack$,$req$                    & \StObjectIdsB     \\
    \StIdle             & \MsgRequestObjIdsNB   & $ack$,$req$                    & \StObjectIdsNB    \\
    \StObjectIdsB       & \MsgReplyObjectIds    & $\langle objectId \rangle$     & \StIdle           \\
    \StObjectIdsNB      & \MsgReplyObjectIds    & $\langle objectId \rangle$     & \StIdle           \\
    \StIdle             & \MsgRequestObjects    & $\langle objectId \rangle$     & \StObjects        \\
    \StObjects          & \MsgReplyObjects      & $\langle object \rangle$       & \StIdle           \\
    \StIdle             & \MsgDone              &                                & \StDone           \\
  \end{tabular}
...
Table~\ref{table:object-diffusion-size-limits} specifies how many bytes can be
sent in a given state; indirectly, this limits the payload size of each
message.  If a space limit is violated, the connection SHOULD be torn down.
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{l|r}
      ...
      \StInit        & \texttt{65535} \\
      \StIdle        & \texttt{65535} \\
      \StObjectIdsB  & \texttt{2500000} \\
      \StObjectIdsNB & \texttt{2500000} \\
      \StObjects     & \texttt{2500000} \\
    \end{tabular}
    \caption{size limits per state}
...
Table~\ref{table:object-diffusion-timeouts} specifies message timeouts in
a given state.  If a timeout is violated, the connection SHOULD be torn down.
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{l|r}
      ...
      \StIdle        & - \\
      \StObjectIdsB  & - \\
      \StObjectIdsNB & \texttt{10}s \\
      \StObjects     & \texttt{10}s \\
    \end{tabular}
    \caption{timeouts per state}
...
\subsection{Peras instantiations}

\begin{table}[h!]
  \begin{center}
    \begin{tabular}{l|c|r|r}
      \header{instantiation}       & \header{mini-protocol} & \header{max.\ outstanding} & \header{ingress buffer} \\
                                   & \header{number}        & \header{identifiers}       & \header{size in bytes}  \\\hline
      Peras certificate diffusion  & $16$                   & $10$                       & $220\,000$ \\
      Peras vote diffusion         & $17$                   & $50$                       & $55\,000$  \\
    \end{tabular}
    \caption{Peras instantiations of the Object-Diffusion mini-protocol}
    \label{table:peras-instantiations}
  \end{center}
\end{table}

\noindent
The limits on the number of outstanding identifiers, and thus the ingress
buffer sizes derived from them (see
table~\ref{table:node-to-node-ingress-buffer-limits}), are provisional; they
are tracked in
\href{https://github.com/tweag/cardano-peras/issues/97}{\textit{Tracking
magic numbers to be tuned}} (\texttt{tweag/cardano-peras\#97}).

\subsection{CDDL encoding specification}\label{object-diffusion-cddl}
\lstinputlisting[style=cddl]{../../cardano-diffusion/protocols/cddl/specs/object-diffusion.cddl}
...
\protocolhaddockref{Ouroboros.Network.Protocol.LocalStateQuery.Type}{ouroboros-network/protocols/Ouroboros-Network-Protocol-LocalStateQuery-Type.html\#t:LocalStateQuery}\\
...
\protocolhaddockref{Ouroboros.Network.Protocol.LocalTxMonitor.Type}{ouroboros-network/protocols/Ouroboros-Network-Protocol-LocalTxMonitor-Type.html\#t:LocalTxMonitor}\\
...
\begin{description}
  ...
  \item[\MsgHasTx{ $txid$}] The client checks whether the server knows of a particular
...
\begin{figure}[h]
  \begin{tabular}{l|l|l|l}
    ...
    \StAcquired            & \MsgHasTx             & $txid$                                   & \StBusy\ \HasTx\\
    \StBusy\ \HasTx        & \MsgReplyHasTx        & Bool                                     & \StAcquired\\
...
Mini-protocols which follow a request-response pattern are amenable to pipelining
such that pipelining becomes a feature of the client implementation and does not
require any modifications to the server implementation.
...
\haddockref{Cardano.Network.NodeToNode}{cardano-diffusion/Cardano-Network-NodeToNode}\newline
\haddockref{Cardano.Network.NodeToNode.Version}{cardano-diffusion/api/Cardano-Network-NodeToNode-Version}\newline
...
\begin{itemize}
  ...
  \item \textit{object-diffusion mini-protocol}, instantiated for Peras
    certificates and Peras votes; from \texttt{NodeToNodeV\_16}, and only if
    \textit{peras support} was negotiated (section~\ref{object-diffusion-protocol})
\end{itemize}
...
\begin{figure}[h]
  \begin{center}
    \begin{tabular}{l|l}
      ...
      \texttt{NodeToNodeV\_16} & Experimental, support Peras mini-protocols\\
    \end{tabular}
...
\begin{table}[ht]
  \begin{center}
    \begin{tabular}{l|c}
      ...
      \hyperref[peer-sharing-protocol]{Peer-Sharing} \small{(if negotiated)}          & $10$ \\
      \hyperref[object-diffusion-protocol]{Peras certificate diffusion} \small{(if negotiated)} & $16$ \\
      \hyperref[object-diffusion-protocol]{Peras vote diffusion} \small{(if negotiated)}        & $17$ \\
    \end{tabular}
  \end{center}
...
\begin{table}[ht]
  \begin{center}
    \begin{tabular}{l|r}
      ...
      \hyperref[block-fetch-protocol]{Block-Fetch}      &  $23\,068\,694$ \\
      ...
      \hyperref[object-diffusion-protocol]{Peras certificate diffusion} &  $220\,000$ \\
      \hyperref[object-diffusion-protocol]{Peras vote diffusion}        &  $55\,000$ \\
    \end{tabular}
...
The two Peras limits are derived from the maximal number of unacknowledged
object identifiers (see table~\ref{table:peras-instantiations}) and are
provisional; they are tracked in
\href{https://github.com/tweag/cardano-peras/issues/97}{\textit{Tracking
magic numbers to be tuned}} (\texttt{tweag/cardano-peras\#97}).

\section{Node-to-client protocol}
\label{section:node-to-client-protocol}
\haddockref{Cardano.Network.NodeToClient}{cardano-diffusion/Cardano-Network-NodeToClient}\newline
\haddockref{Cardano.Network.NodeToClient.Version}{cardano-diffusion/api/Cardano-Network-NodeToClient-Version}\newline
...
\begin{figure}[h]
  \begin{center}
    \begin{tabular}{l|l}
      ...
      \texttt{NodeToClientV\_22} & support SRV records in \texttt{GetBigLedgerPeerSnapshot} \\
      \texttt{NodeToClientV\_23} & added \texttt{QueryDRepsDelegations}; \\
                                 & \texttt{LedgerPeerSnapshot} CBOR encoding contains block hash and network magic \\
    \end{tabular}
