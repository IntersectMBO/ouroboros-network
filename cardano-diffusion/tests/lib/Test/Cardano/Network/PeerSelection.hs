...
-- TODO:
-- we should have `Test.Ouroboros.Network.PeerSelection` tests in
...
isEmptyEnv GovernorMockEnvironment {
 ...
 && (PublicRootPeers.null Cardano.ExtraPeers.nullAll publicRootPeers
      || case consensusMode of
           PraosMode ->
    ...
    let ioSimTrace = runGovernorInMockEnvironment env {
        ...
        reachable  = peerShareReachablePeers peerGraph
                       (    LocalRootPeers.keysSet localRootPeers
                         <> PublicRootPeers.toSet Cardano.ExtraPeers.toSet publicRootPeers
                       )
...
prop_governor_target_root_below env =
    let events = Signal.eventsFromListUpToTime (Time (10 * 60 * 60))
        ...
        govTargetsSig =
          selectGovState (seed env)
                         (targetNumberOfRootPeers . Governor.targets)
...
prop_governor_target_established_big_ledger_peers (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govBigLedgerPeersSig =
          selectGovState (seed env)
                         (PublicRootPeers.getBigLedgerPeers . Governor.publicRootPeers)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
        ...
        envKnownPeersSig =
          ...
          . Signal.selectEvents
              (\case
                  ...
                  TracePublicRootsResults x _ _    -> Just (PublicRootPeers.toSet Cardano.ExtraPeers.toSet x)
...
prop_governor_target_known_2_opportunity_taken (MaxTime maxTime) env =
    ...
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govTargetsSig =
          selectGovState (seed env)
                         (targetNumberOfKnownPeers . Governor.targets)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
...
prop_governor_target_known_5_no_shrink_below (MaxTime maxTime) env =
    let events :: Events (TestTraceEvent ExtraState PeerTrustable (Cardano.ExtraPeers PeerAddr))
        ...
        bootstrapPeersSig =
          selectGovState (seed env)
                         (Cardano.PublicRootPeers.getBootstrapPeers . Governor.publicRootPeers)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
...
prop_governor_target_known_5_no_shrink_big_ledger_peers_below (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govTargetsSig =
          selectGovState (seed env)
                         (targetNumberOfKnownBigLedgerPeers . Governor.targets)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
...
prop_governor_target_established_below (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govTargetsSig =
          selectGovState (seed env)
                         (targetNumberOfEstablishedPeers . Governor.targets)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
        ...
        govInProgressPromoteColdSig =
          selectGovState (seed env)
                         (dropBigLedgerPeers Governor.inProgressPromoteCold)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
...
prop_governor_target_established_big_ledger_peers_below (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govTargetsSig =
          selectGovState (seed env)
                         (targetNumberOfEstablishedBigLedgerPeers . Governor.targets)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
        ...
        govInProgressIneligibleSig =
          selectGovState (seed env)
                         (takeBigLedgerPeers Governor.inProgressPromoteCold)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
...
prop_governor_target_active_below (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govInProgressIneligibleSig =
          selectGovState (seed env)
                         (\psState -> Set.unions [ Governor.inProgressDemoteToCold psState
...
prop_governor_target_active_big_ledger_peers_below (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govTargetsSig =
          selectGovState (seed env)
                         (targetNumberOfActiveBigLedgerPeers . Governor.targets)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
        ...
        govActivePeersSig =
          selectGovState (seed env)
                         (takeBigLedgerPeers Governor.activePeers)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
...
prop_governor_target_established_above (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govDemotionsInProgressSig =
          selectGovState (seed env)
                         (\psState -> Set.unions [ Governor.inProgressDemoteWarm psState
...
prop_governor_target_active_above (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govInProgressIneligibleSig =
          selectGovState (seed env)
                         (\psState -> Set.union (Governor.inProgressDemoteToCold psState)
...
prop_governor_target_active_local_below (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govInProgressPromoteWarmSig =
          selectGovState (seed env)
                         Governor.inProgressPromoteWarm
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
...
prop_governor_no_non_trustable_peers_before_caught_up_state env =
    let events = Signal.eventsFromListUpToTime (Time (10 * 60 * 60))
        ...
        govHasOnlyBootstrapPeers =
          selectGovState (seed env)
                         (Cardano.ExtraState.hasOnlyBootstrapPeers . Governor.extraState)
                         (Cardano.ExtraState.empty (consensusMode env)
                                                   (NumberOfBigLedgerPeers 0))
                         Cardano.ExtraPeers.empty
                         events
...
prop_governor_uses_ledger_peers env =
    let events = Signal.eventsFromListUpToTime (Time (10 * 60 * 60))
        ...
        govPublicRootPeersResultsSig =
            Signal.fromEventsWith (PublicRootPeers.empty Cardano.ExtraPeers.empty)
          . Signal.selectEvents
              (\case
...
--
_governorFindingPublicRoots :: TestSeed
                            -> Int
                            ...
                            -> ConsensusMode
                            -> IO Void
_governorFindingPublicRoots (TestSeed seed') targetNumberOfRootPeers readDomains readUseBootstrapPeers
    ...
    debugStateVar <- newTVarIO $
      emptyPeerSelectionState (mkStdGen seed')
    ...
    publicRootPeersProvider
      ...
      (mkStdGen seed') $ \requestPublicRootPeers -> do
