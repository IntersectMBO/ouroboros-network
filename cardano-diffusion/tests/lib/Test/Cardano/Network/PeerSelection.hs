...
prop_governor_target_established_below (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        -- There are no opportunities if we're at or above target
        --
        promotionOpportunity target known established recentFailures inProgressCold
...
prop_governor_target_established_local (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govEstablishedTargetsSig :: Signal Int
        govEstablishedTargetsSig =
          selectGovState (targetNumberOfEstablishedPeers . Governor.targets)
            (Cardano.ExtraState.empty (consensusMode env) (NumberOfBigLedgerPeers 0)) Cardano.ExtraPeers.empty
            events
        ...
        govInProgressPromoteBigColdSig :: Signal (Set PeerAddr)
        govInProgressPromoteBigColdSig =
          selectGovState
        ...
        promotionOpportunities :: Signal (Set PeerAddr)
        promotionOpportunities =
          (\target local established estBig recentFailures promoteCold promoteBigCold ->
                Set.unions
                  [opportunity
                  | (_, WarmValency warmTarget, group) <- LocalRootPeers.toGroupSets local
                  , let opportunity
                          | Set.size established + Set.size promoteCold >= target = Set.empty
                          | Set.size (Set.intersection
                                       group (established `Set.union` estBig)) >= warmTarget = Set.empty
                          | otherwise = group
                  ]
                  Set.\\ Set.unions [established, estBig, recentFailures, promoteCold, promoteBigCold]
          ) <$> govEstablishedTargetsSig
            <*> govLocalRootPeersSig
            <*> govEstablishedPeersSig
            <*> govEstablishedBigPeersSig
...
prop_governor_target_active_local_below (MaxTime maxTime) env =
    let events = Signal.eventsFromListUpToTime maxTime
        ...
        govInProgressPromoteWarmSig =
          selectGovState (dropBigLedgerPeers Governor.inProgressPromoteWarm)
                         (Cardano.ExtraState.empty (consensusMode env) (NumberOfBigLedgerPeers 0)) Cardano.ExtraPeers.empty
                         events

        govInProgressPromoteBigWarmSig :: Signal (Set PeerAddr)
        govInProgressPromoteBigWarmSig =
          selectGovState (takeBigLedgerPeers Governor.inProgressPromoteWarm)
                         (Cardano.ExtraState.empty (consensusMode env) (NumberOfBigLedgerPeers 0)) Cardano.ExtraPeers.empty
                         events
        ...
        promotionOpportunities :: Signal (Set PeerAddr)
        promotionOpportunities =
          (\target local established active actBig
            recentFailures promoteWarm promoteBigWarm inProgressIneligible ->
                Set.unions
                  [opportunity
                  | (HotValency hotTarget, _, group) <- LocalRootPeers.toGroupSets local
                  , let opportunity
                          | Set.size active + Set.size promoteWarm >= target = Set.empty
                          | Set.size (group `Set.intersection` (active `Set.union` actBig)) >= hotTarget = Set.empty
                          | otherwise = group `Set.intersection` established
                  ]
                  Set.\\ Set.unions [active, actBig, recentFailures, promoteWarm, promoteBigWarm, inProgressIneligible]
          ) <$> govActiveTargetsSig
            <*> govLocalRootPeersSig
            <*> govEstablishedPeersSig
            <*> govActivePeersSig
