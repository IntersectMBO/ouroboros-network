...
module Test.Cardano.Network.PeerSelection.MockEnvironment
  ( PeerGraph (..)
...
mockPeerSelectionActions' tracer
  ...
  where
    ...
    establishPeerConnection :: IsBigLedgerPeer -> DiffusionMode -> Provenance -> PeerAddr -> PeerTrustable -> m (PeerConn m)
    establishPeerConnection _ _ _ peeraddr _ = do
...
traceAssociationMode interfaces actions = Tracer $ \(TraceGovernorState _ _ st) -> do
    associationMode <- atomically $ readAssociationMode
                                           ...
                                           (Cardano.bootstrapPeersFlag (Governor.extraState st))
