  ...
  where
    ...
    pad req (BlockingReply xs) =
        let target = fromIntegral (getNumTxIdsToReq req) + fromIntegral n :: Int
            need   = target - NonEmpty.length xs
...
validImpairment imp =
  ...
  && impairOmitProb imp <= 1
  && maybe True (> 0) (impairBodyDelay imp)
...
shrinkImpairment imp = List.nub $
  ...
  ++ [ imp { impairExtraTxIds = a }        | a <- shrink (impairExtraTxIds imp) ]
