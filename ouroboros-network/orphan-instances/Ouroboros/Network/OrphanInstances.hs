...
import Ouroboros.Network.ConnectionManager.State (ConnMap (..), ConnStateId (..),
           LocalAddr (..))
...
import Ouroboros.Network.PeerSelection.Governor.Types (
           AssociationMode (..), DebugPeerSelectionState (..),
           PeerSharingResult (..), TracePeerSelection (..))
...
instance (Show addr, ToJSON addr)
  ...
  toJSON (Server.TrAcceptError exception)         =
    object [ "kind" .= String "AcceptErroor"
