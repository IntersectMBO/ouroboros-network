...
sharedTxStateInvariant SharedTxState {
    ...
    .&&. counterexample "PeerTxState invariant violation"
         (getAllProperty $ foldMap (\ps -> AllProperty
...
data TxMask tx = TxAvailable SizeInBytes TxStatus
                 -- ^ available txid with its size, the Bool indicates if it's
                 -- in-flight or not
               | TxBuffered tx

-- maximal tx size is 65536
--
-- TODO: we should generate it. Note that the limit is needed in tests.
--
maxTxSize :: SizeInBytes
maxTxSize = 65536

instance Arbitrary tx => Arbitrary (TxMask tx) where
    arbitrary = oneof [ TxAvailable . SizeInBytes . fromIntegral
                          <$> chooseInt (0, fromIntegral maxTxSize)
                          <*> arbitrary
...
mkArbPeerTxState mempoolHasTxFun txIdsInflight unacked txMaskMap =
    ArbPeerTxState
      PeerTxState { unacknowledgedTxIds = StrictSeq.fromList unacked,
                    ...
                    requestedTxsInflightSize,
                    unknownTxs }
      (Map.keysSet inflightMap)
      bufferedMap
  where
    mempoolHasTx   = apply mempoolHasTxFun
    availableTxIds = Map.fromList
                   [ (txid, size) | (txid, TxAvailable size _) <- Map.assocs txMaskMap
    ...
    requestedTxsInflightSize = fold inflightMap
    ...
    inflightMap = Map.fromList
                [ (txid, size)
                | (txid, TxAvailable size Inflight) <- Map.assocs txMaskMap
                , not (mempoolHasTx txid)
                ]
...
genArbPeerTxState mempoolHasTxFun maxTxIdsInflight = do
    ...
    txMap <- Map.fromList <$> traverse (\txid -> (txid,) <$> arbitrary) (nub unacked)
        ...
        st = fixupSharedTxState
               ...
               SharedTxState {
                 ...
                 inflightTxs = foldl' (Map.unionWith (+)) Map.empty
                             [ Map.fromSet (const 1) arbInflightSet
...
instance Arbitrary ArbSharedTxState where
  arbitrary = do
    ...
    (mempoolHasTx, _, sharedTxState) <- genSharedTxState maxTxIdsInflight
...
prop_SharedTxState_shrinker =
    getAllProperty
  . foldMap (\(ArbSharedTxState _ st) -> AllProperty $ sharedTxStateInvariant st)
  . shrink
  . getFixed
...
instance  Arbitrary ArbReceivedTxIds where
    arbitrary = do
        Small maxTxIdsInflight <- arbitrary
        (mempoolHasTxFun, (peeraddr, ps), st) <- genSharedTxState maxTxIdsInflight
        txsToAck <- arbitrary
    ...
    -- TODO: improve shrinking
    shrink (ArbReceivedTxIds mempoolHasTxFun txs peeraddr ps st) =
      [ ArbReceivedTxIds mempoolHasTxFun txs' peeraddr ps st
      | txs' <- shrink txs
      ]
      ++
      [ ArbReceivedTxIds mempoolHasTxFun txs peeraddr ps
        (fixupSharedTxState (apply mempoolHasTxFun) st'')
      | st' <- shrinkSharedTxState (apply mempoolHasTxFun) st
      , let st'' = st' { peerTxStates = Map.insert peeraddr ps (peerTxStates st') }
      ]
      ++
      [ ArbReceivedTxIds
...
prop_acknowledgeTxIds (ArbReceivedTxIds _mempoolHasTxFun _txs _peeraddr ps st) =
    case TXS.acknowledgeTxIds st ps of
      (numTxIdsToAck, TXS.RefCountDiff { TXS.txIdsToAck }, ps') ->
...
prop_receivedTxIdsImpl (ArbReceivedTxIds mempoolHasTxFun txs peeraddr ps st) =
    case TXS.receivedTxIdsImpl (apply mempoolHasTxFun)
      ...
      (noToAck, st') ->
        ...
        .&&. -- progress property of unacknowledged txs
             counterexample "acknowledged property violation"
             (  let unacked  = toList $ unacknowledgedTxIds ps
                    unacked' = toList $ unacknowledgedTxIds ps'
                in
                     -- we are acknowledging all txids from `st` which are
                     -- buffered or belong to the mempool. Note that this is
                     -- slightly weaker what the short phrase below states.
                     counterexample ("not all txids that could be acknowledged are acknowledged"
                                     ++ show (unacked, unacked'))
                     (dropWhile
                         (\txid ->
                              -- buffered txs can be acknowledged
                              txid `Map.member` bufferedTxs st
                              -- unknown txids can be acknowledged
                           || txid `Set.member` unknownTxs (peerTxStates st Map.! peeraddr)
                              -- received txids which are in the mempool can be acknowledged
                           || txid `Map.member` txidMap && apply mempoolHasTxFun txid
                         )
                         unacked
                       `isPrefixOf`
                       unacked'
        ...
        .&&. -- `hasTxIdsToAcknowledge` returns `False` after all `txid`s where
             -- acknowledged
             counterexample "hasTxIdsToAcknowledge"
             ( not $ TXS.hasTxIdsToAcknowledge peeraddr
                                               st'
                                               (TXS.unacknowledgedTxIds ps')
             )
  where
    txidSeq = StrictSeq.fromList (getTxId <$> txs)
    txidMap = Map.fromList [ (getTxId tx, getTxSize tx) | tx <- txs ]

    labelNoToAck :: Int -> String
    labelNoToAck x = "noToAck " ++ labelInt 10 2 x
...
prop_receivedTxIdsImpl_nothunks (ArbReceivedTxIds mempoolHasTxFun txs peeraddr _ st) =
    case TXS.receivedTxIdsImpl (apply mempoolHasTxFun)
                               peeraddr 0 txidSeq txidMap st of
      (_, !st') -> case unsafeNoThunks st' of
        Nothing  -> property True
        Just ctx -> counterexample (show ctx) False
  where
    txidSeq = StrictSeq.fromList (getTxId <$> txs)
...
prop_hasTxIdsToAcknowledge :: ArbReceivedTxIds
                           -> Property
prop_hasTxIdsToAcknowledge (ArbReceivedTxIds mempoolHasTxFun _txs peeraddr ps st) =
    case TXS.receivedTxIdsImpl (apply mempoolHasTxFun)
                               peeraddr 0 txidSeq txidMap st of
      (noToAck, _) ->
        -- hasTxIdsToAcknowledge
        counterexample "hasTxIdsToAcknowledge"
        ( let hasTxIdsToAck  = TXS.hasTxIdsToAcknowledge peeraddr st (TXS.unacknowledgedTxIds ps)
              hasTxIdsToAck' = noToAck > 0
          in hasTxIdsToAck === hasTxIdsToAck'
        )
  where
    -- we ignore txids which were received, since we just want to check that
    -- the current state has any `txid`s to acknowledge.
    txidSeq = StrictSeq.empty
    txidMap = Map.empty
...
prop_collectTxs_generator (ArbCollectTxs _ requestedTxIds receivedTxs peeraddr
       ...
       (fold (availableTxIds `Map.restrictKeys` requestedTxIds) <= requestedTxsInflightSize)
...
prop_collectTxs_shrinker (Fixed txs) =
    getAllProperty $ foldMap (\a@(ArbCollectTxs _ _ _ _ _ st) ->
                               AllProperty . counterexample (show st) $
                                     f a =/= f txs
                                .&&. sharedTxStateInvariant st
                             ) (shrink txs)
  where
...
prop_collectTxsImpl (ArbCollectTxs _mempoolHasTxFun txidsRequested txsReceived peeraddr ps st) =
    ...
    case TXS.collectTxsImpl peeraddr txidsRequested txsReceived st of
      ((noToAck, _txReady), st') ->
        let !ps' = peerTxStates st' Map.! peeraddr in

        label ("number of txids to acknowledge " ++ labelInt 10 2 noToAck) $
        ...
        label ("number of txids received "       ++ labelInt 10 2 (Map.size txsReceived)) $
             ...
             -- InboundState invariant
             counterexample
               (  "TxIds to acknowledge: " ++ show noToAck ++ "\n"
               ++ "InboundState invariant violation:\n" ++ show st' ++ "\n"
               ++ show ps'
               )
               (sharedTxStateInvariant st')
               ...
               in
                 ...
                 (dropWhile
                     (\txid ->
                          ...
                          -- unknown txids can be acknowledged
                       || txid `Set.member` unknownTxs (peerTxStates st Map.! peeraddr)
                          -- all requested txs (weather received or
                          -- not) can be acknowledged
                       || txid `Set.member` txidsRequested)
                     unacked
                   `isPrefixOf`
                   unacked'
                 )
