...
tests = testGroup "Ouroboros.Network.TxSubmission"
  ...
  , testGroup "Decisions"
    [ testGroup "ArbDecisionContexts"
      ...
      , testProperty "shrinker"                prop_ArbDecisionContexts_shrinker
      ]
...
instance  Arbitrary ArbReceivedTxIds where
    ...
    shrink (ArbReceivedTxIds mempoolHasTxFun txs peeraddr ps st) =
      ...
      , let st'' = st' { peerTxStates = Map.insert peeraddr ps (peerTxStates st') }
            txs' = take (fromIntegral $ requestedTxIdsInflight ps) txs
      ]
      ++
      [ ArbReceivedTxIds
