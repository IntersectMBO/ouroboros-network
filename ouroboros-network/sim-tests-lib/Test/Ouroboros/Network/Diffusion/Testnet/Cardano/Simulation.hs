...
instance Show NodeArgs where
    show NodeArgs { naSeed, naDiffusionMode, naMbTime, naBootstrapPeers,
                    naPublicRoots, naAddr, naPeerSharing, naLocalRootPeers,
                    naPeerTargets, naDNSTimeoutScript, naDNSLookupDelayScript,
                    naChainSyncExitOnBlockNo, naChainSyncEarlyExit,
                    naFetchModeScript, naConsensusMode, naTxs } =
      unwords [ "NodeArgs"
              , "(" ++ show naSeed ++ ")"
...
genNodeArgs :: SimArgs
            ...
            -> [Tx Int]
            -> Gen NodeArgs
genNodeArgs SimArgs {saSlot, saQuota} relays minConnected localRootPeers relay txs = flip suchThat hasUpstream $ do
...
genDiffusionScript genLocalRootPeers
  ...
  where
    ...
    go :: SimArgs -> (RelayAccessInfo, [Tx Int]) -> Gen (NodeArgs, [Command])
