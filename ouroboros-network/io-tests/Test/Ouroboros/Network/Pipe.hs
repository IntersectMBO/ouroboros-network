    ...
    bracket
      ...
      $ \((hndRead1, hndWrite1), (hndRead2, hndWrite2)) -> do
        ...
        _ <- async $ do
              ...
              withAsync (Mx.run (Mx.MuxTracerBundle nullTracer nullTracer) clientMux clientBearer) $ \aid -> do
        ...
        _ <- async $ do
              ...
              withAsync (Mx.run (Mx.MuxTracerBundle nullTracer nullTracer) serverMux serverBearer) $ \aid -> do
                _ <- atomically $ runFirstToFinish $ foldMap FirstToFinish resOps
                Mx.stop serverMux
                wait aid
