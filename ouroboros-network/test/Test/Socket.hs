...
prop_socket_send_recv serverAddr f xs = do
    ...
    res <-
      withServerNode
        ...
        (simpleSingletonVersions NodeToNodeV_1 (NodeToNodeVersionData 0) DictVersion serverApp)
        $ \_ -> do
          connectTo
            (\DictVersion -> encodeTerm)
            (\DictVersion -> decodeTerm)
            (simpleSingletonVersions NodeToNodeV_1 (NodeToNodeVersionData 0) DictVersion clientApp)
            serverAddr
...
demo chain0 updates = do
    ...
    withServerNode
      ...
      (\DictVersion -> decodeTerm)
      (\DictVersion -> acceptEq)
      (simpleSingletonVersions (0::Int) (NodeToNodeVersionData 0) DictVersion producerApp)
      $ \_ -> do
      withAsync (connectTo (\DictVersion -> encodeTerm) (\DictVersion -> decodeTerm) (simpleSingletonVersions (0::Int) (NodeToNodeVersionData 0) DictVersion consumerApp) producerAddress) $ \_connAsync -> do
        void $ fork $ sequence_
            [ do
