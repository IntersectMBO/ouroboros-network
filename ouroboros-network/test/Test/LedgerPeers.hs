...
data LedgerPools = LedgerPools [(PoolStake, NonEmpty RelayAddress)] deriving Show
    ...
    arbitrary = do
      ...
      where
        ...
        calculateRelativeStake sps =
            ...
            map (\p -> ((fromIntegral $ spStake p) % (fromIntegral totalStake), spRelay p)) sps 
...
prop_pick (LedgerPools lps) count seed =
    ...
    ioProperty $ do
        ...
        case tr' of
             ...
             SimReturn (_, peers) _trace -> do
                 -- printf "Log: %s\n" (intercalate "\n" _trace)
                 if null lps then return $ property $ null peers
                             else return $ length peers === fromIntegral count
