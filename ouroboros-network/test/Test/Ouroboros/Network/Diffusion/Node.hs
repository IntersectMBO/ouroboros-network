...
data Interfaces m = Interfaces
    ...
    , iNtnDomainResolver :: [DomainAddress] -> m (Map DomainAddress (Set NtNAddr))
...
data Arguments m = Arguments
    ...
    , aReadLocalRootPeers   :: STM m [(Int, Map RelayAddress PeerAdvertise)]
    , aReadPublicRootPeers  :: STM m [RelayAddress]
...
run blockGeneratorArgs limits ni na =
    Node.withNodeKernelThread blockGeneratorArgs
      $ \ nodeKernel nodeKernelThread -> do
        dnsStdGenVar <- newTVarIO dnsStdGen
        ...
        let -- diffusion interfaces
            ...
            interfaces = Diff.P2P.Interfaces
              ...
              , Diff.P2P.diDnsActions            = mockDNSActions (iDomainMap ni)
                                                                  dnsStdGenVar
              }
  ...
  where
    ...
    (diffStgGen, (dnsStdGen, keepAliveStdGen)) = split <$> split (iRng ni)
    ...
    args = Diff.Arguments
      ...
      , Diff.daAcceptedConnectionsLimit
                             = aAcceptedLimits na
      , Diff.daDiffusionMode = aDiffusionMode na
      }
