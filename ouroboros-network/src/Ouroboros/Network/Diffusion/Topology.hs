...
rootConfigToRelayAccessPoint RootConfig { rootAccessPoints, rootAdvertise, followBack } =
    [ (ap, rootAdvertise, followBack) | ap <- rootAccessPoints ]
...
producerAddresses NetworkTopology { localRootPeersGroups
  ...
  ( map (\lrp -> ( hotValency lrp
                 , warmValency lrp
                 , Map.fromList
                 . map (\(addr, peerAdvertise, followBack) ->
                         ( addr
                         , LocalRootConfig {
