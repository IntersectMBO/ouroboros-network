    ...
    fn ps = (st'', ps'')
      where
        ...
        -- bufferedTxs' = bufferedTxs st
                    -- KK XXX <> Map.map Just receivedTxs

        bufferedTxs' = bufferedTxs st -- <> Map.map (const Nothing) receivedTxs

        downloadedTxs' = downloadedTxs ps <> receivedTxs
        -- limboTxs' = Map.unionWith (+) (limboTxs st) $ Map.map (const 1) receivedTxs
