...
headerMetricTracer
    :: forall m p.
       ( MonadSTM m )
    => PeerMetrics m p
    -> Tracer (STM m) (WithPeer (ConnectionId p) (SlotNo, Time))
headerMetricTracer PeerMetrics{headerMetrics} =
    mapPeerAddr remoteAddress
    `contramap`
...
fetchedMetricTracer
    :: forall m p.
       ( MonadSTM m )
    => PeerMetrics m p
    -> Tracer (STM m) (WithPeer (ConnectionId p)
                                ( SizeInBytes
                                , SlotNo
                                , Time
                                ))
fetchedMetricTracer PeerMetrics{fetchedMetrics} =
    (\(WithPeer con (bytes, slot, time)) -> WithPeer (remoteAddress con, bytes)
                                                     (slot, time))
    `contramap`
...
metricsTracer
    ...
    -> Tracer (STM m) (WithPeer p (SlotNo, Time))
metricsTracer metricsVar = Tracer $ \(WithPeer !peer (!slot, !time)) -> do
