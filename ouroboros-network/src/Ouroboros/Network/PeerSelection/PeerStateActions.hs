  ...
  where
    ...
    establishPeerConnection jobPool remotePeerAddr =
      ...
      bracketOnError
        ...
        $ \peerStateVar -> do
          res <- includeOutboundConnection spsConnectionManager remotePeerAddr
          case res of
            Connected connectionId@ConnectionId { localAddress, remoteAddress }
                      (Handle mux muxBundle controlMessageVarBundle) -> do
    ...
    closePeerConnection
      ...
      case res of
        Nothing -> do
          unregisterOutboundConnection spsConnectionManager (remoteAddress pchConnectionId)
          atomically (writeTVar pchPeerState (PeerStatus PeerCold))
          traceWith spsTracer (PeerStatusChangeFailure
