...
withPeerStateActions timeout
  ...
  where
    peerMonitoringLoop
      :: PeerConnectionHandle muxMode peerAddr ByteString m a b
      -> m ()
    peerMonitoringLoop pch@PeerConnectionHandle { pchPeerState, pchAppHandles } = do
        ...
        case r of
          ...
          WithSomeProtocolTemperature (WithHot MuxApplicationError{}) ->
            atomically (writeTVar pchPeerState (PeerStatus PeerCold))
          WithSomeProtocolTemperature (WithWarm MuxApplicationError{}) ->
            atomically (writeTVar pchPeerState (PeerStatus PeerCold))
          WithSomeProtocolTemperature (WithEstablished MuxApplicationError{}) ->
            atomically (writeTVar pchPeerState (PeerStatus PeerCold))
    ...
    establishPeerConnection :: JobPool m ()
                            -> peerAddr
          ...
          case muxPromise of
            MuxRunning connectionId@ConnectionId { localAddress, remoteAddress }
              ...
              JobPool.forkJob jobPool
                              (Job (peerMonitoringLoop connHandle)
                                   (const ())
...
startProtocols tok PeerConnectionHandle { pchMux, pchAppHandles } = do
  ...
  where
    ...
    awaitSTM = foldr (\(miniProtocolNum, stm) acc ->
                       (either (MuxApplicationError miniProtocolNum) (const MuxApplicationSuccess)
                         <$> stm)
                       `orElse` acc)
                     retry
