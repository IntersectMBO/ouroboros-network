...
import           Ouroboros.Network.PeerSelection.Bootstrap
                     (isBootstrapPeersEnabled, isInSensitiveState,
    ...
    Guarded Nothing $ do
      ...
      return $ \_ ->
        let -- If peer happens to already be present in the Known Peer set
            ...
            newEntry    = Map.singleton addr (Just ps, Just DoAdvertisePeer)
...
connections PeerSelectionActions{
  ...
  where
    bigLedgerPeersSet = PubliRootPeers.getBigLedgerPeers publicRootPeers
...
localRoots actions@PeerSelectionActions{ readLocalRootPeers
  ...
  | isNodeAbleToMakeProgress bootstrapPeersFlag ledgerStateJudgement hasOnlyBootstrapPeers =
    Guarded Nothing $ do
      ...
      let inSensitiveState = isInSensitiveState bootstrapPeersFlag ledgerStateJudgement
...
-- order to do this, when bootstrap peers are disabled we should update the
-- ledger state judgement value to 'YoungEnough'; and 'hasOnlyBootstrapPeers'
-- to 'False'.
--
...
-- 'TooOld' we set all other targets to 0 and wait the governor waits for all
-- active connections to drop and then set the targets for bootstrap peers.
...
-- | If the node just got in a fallback state, the node just had its targets
-- adjusted to get rid of all peers. This jobs monitors the node state and when
-- it has arrived to a clean state it sets the 'hasOnlyBootstrapPeers' flag on
-- which will unblock the 'localRoots' and 'targetPeers' monitoring actions.
--
...
--
waitForOnlyBootstrapPeers :: ( MonadSTM m
                             , Ord peeraddr
                             )
                          => PeerSelectionState peeraddr peerconn
                          -> Guarded (STM m) (TimedDecision m peeraddr peerconn)
waitForOnlyBootstrapPeers st@PeerSelectionState{
                               ledgerStateJudgement
                             , bootstrapPeersFlag
                             , knownPeers
                             , localRootPeers
  ...
  | isInSensitiveState bootstrapPeersFlag ledgerStateJudgement
