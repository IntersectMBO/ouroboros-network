...
--
withResource' :: (Monad m, MonadDelay m)
              => Tracer m err
              -> NonEmpty DiffTime
...
--
data DNSActions resolvConf resolver exception m = DNSActions {
    ...
    -- @GetNetworkParams@ win32 api call to get the list of default dns servers.
    --
    dnsNewResolverResource   :: resolvConf -> Resource m (DNSorIOError exception) resolver,
#endif
    ...
    -- additional timeout.
    --
    dnsLookupAWithTTL        :: TimeoutFn m -> resolvConf -> resolver -> DNS.Domain -> m (Either DNS.DNSError [(IPv4, DNS.TTL)])
  }
...
--
localRootPeersProvider
  :: forall m resolvConf resolver exception.
  ...
  -> DNSActions resolvConf resolver exception m
  -> m Void
...
publicRootPeersProvider
  :: forall resolvConf resolver exception a m.
...
resolveDomainAddresses
  ...
  -> TimeoutFn m
  -> DNS.ResolvConf
  -> DNSActions DNS.ResolvConf DNS.Resolver exception m
  -> [DomainAddress]
