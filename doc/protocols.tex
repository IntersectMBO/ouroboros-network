...
\item[Version 0.7, November 29, 2018] Consolidated requirements into this document
\end{description}
...
architecture of the original Cardano Main-net release was entirely homogeneous:
...
\item family of protocol variations for different environments: STM, IO, local
  IPC, remote IPC
  \njd{\\noting different levels of implicit trust here - we need
    to think whether the STM environment is one we treat differently (e.g. trust
    forwarding only correctly signed blocks reducing the checks in the consumer
    end) or do we assume even shared process nodes are potentially misbehaving
...
\avieth{When a tip-of-chain announcement is received from a peer, and it's valid
  ...
  cut out honest nodes.}

  \avieth{The probability that a node has multiple chains which disagree on the
  ...
  number of slots retained by this forking data structure ($k$) we can
  decrease the probability that the oldest slot will include more than one
  candidate (multiple blocks, or no block, because some maximal chain is known
  ...
  work it into some explicit probabilistic guarantee.}
...
Alternatively, the rule could state that a header is relayed without the
...
\subsubsection{Phase 2, reproducing producer's chain on the consumer's end}
...
\item how many slots could an attacker maintain a fiction with a)
  unfettered power of eclipse; b) $\rho$ adversarial stake at its disposal.
\item what indicators of certainty are available to inform end user?
\end{itemize}
\subsection{Inherent Asymmetry}
\begin{itemize}
...
\item Given that a node is not totally eclipsed (i.e $\exists$
  potentially reachable non-adversarial bearer node) how does this help?
\end{itemize}  
