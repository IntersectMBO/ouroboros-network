...
to replicate the producer's blockchain locally.  It is polymorphic in blocks
as it is designed to support family of Ouroboros protocols, but
also for some clients it will synchronize just headers (node-to-node), for
others it can be used to synchronize actual blockchain (e.g., node-to-wallet).
...
Note, that, for readablitiy, the previous Sections used simplified message identifiers,
for example \tt{RequestNext} instead of \tt{msgRequestNext}, etc.
Both identifiers refere to the same message format.
