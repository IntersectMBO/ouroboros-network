...
\todo{TODO} TODO: This is the first thick red line that does not represent the
densest chain in the window. It seems worthwhile to explain that the thick red
line can mean different things in different diagrams, or even better to list
its more general, consistent definition, if there is one. -NSF
...
\todo{TODO} TODO: update diagram for to be clear wrt ``in or after''. -NSF
...
to date to fall behind again; the reason might be as simple as a user putting
their computer to sleep for a few days. It is therefore important that we can go
