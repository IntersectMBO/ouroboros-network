...
In the implementation, each node follow the chains of its upstream peers as
...
TODO\todo{TODO}: Should we think harder about pre-emptively download blocks?
...
The only thing that could prevent the chain database from adopting a particular
chain is the rollback limit $k$. This suggests the following implementation:

\begin{definition}[Suboptimal chain selection]
...
\begin{center}
\begin{tikzpicture}[yscale=0.5]
\draw (0,0) node{$\bullet$} -- ++(1,1) -- ++(3,0) node{$\bullet$} node[right]{$O$};
\draw (0,0) -- ++(1,-1) -- ++(2,0) node{$\bullet$} node[right]{$S$};
\draw (1,1) node{$\bullet$} -- ++(1,1) -- ++(3,0) node{$\bullet$} node[right]{$O'$};
\end{tikzpicture}
\end{center}

Since we can roll back from $S$ to the intersection point with $O$, we can
...
\draw (0,0) node{$\bullet$} -- ++(1,1) -- ++(3,0) node{$\bullet$} node[right]{$O$};
\draw (0,0) -- ++(1,-1) -- ++(2,0) node{$\bullet$} node[right]{$S$};
\draw (-1,1) node{$\bullet$} -- (0,0);
\draw (-1,1) -- ++(1,1) -- ++(5,0) node{$\bullet$} node[right]{$O'$};
\end{tikzpicture}
\end{center}
...
Suppose that we have two chains $C_0$ and $C_1$. Judging by the headers, $C_0$
...
either they don't see $C_0$ or they are not getting the blocks from it. \todo{Is
  this true?}

\subsection{When the node is behind}
...
TODO\todo{TODO}: Unlike what the report says, we in fact decided we \emph{do}
