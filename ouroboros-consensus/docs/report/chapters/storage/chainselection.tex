...
the consensus protocol (\cref{chain-selection:overview}), but needs to  take
...
Recall from \cref{chain-selection:overview} that while in the literature chain
selection is defined in terms of comparisons between entire chains, we instead
opted to model it in terms of a comparison between the \emph{headers} at the tip
of those chains (or rather, a \emph{view} on those headers defined by the
...
We mentioned in \cref{chain-selection:overview} that consensus imposes a
...
case, we would induce a roll back of more than $k$ blocks (\cref{param:k}).
...
\todo{TODO} TODO: Throughout we are talking about \emph{anchored} framgnents
...
consensus protocol (\cref{chain-selection:overview}). In this section we
...
protocol. \Cref{never-shrink} (\cref{chain-selection}) however means that
we \emph{do} have that
%
\begin{equation}
...
we cannot say anything about chains of equal length). \Cref{prefer-extension}
(\cref{chain-selection:overview}) can now be rephrased as
%
\begin{equation}
