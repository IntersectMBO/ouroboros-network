...
deleteAfterImpl dbEnv@ImmutableDBEnv { _dbTracer } newTip =
  ...
  modifyOpenState dbEnv $ \hasFS@HasFS{..} -> do
    ...
    let newTipEpochSlot     = (blockOrEBBEpochSlot . forgetTipInfo) <$> newTip
...
appendBlockImpl dbEnv slot blockNumber headerHash binaryInfo =
    modifyOpenState dbEnv $ \_dbHasFS@HasFS{..} -> do
      ...
      appendEpochSlot _dbRegistry _dbHasFS _dbChunkInfo _index epochSlot
...
appendEpochSlot registry hasFS chunkInfo index epochSlot blockNumber blockOrEBB headerHash
