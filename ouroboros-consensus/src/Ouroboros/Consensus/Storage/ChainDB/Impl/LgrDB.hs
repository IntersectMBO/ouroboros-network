...
openDB args@LgrDbArgs { lgrHasFS = lgrHasFS@(SomeHasFS hasFS), .. } replayTracer immutableDB getBlock = do
    ...
    --
    -- - Push onto the ledger DB all the ledger states that resulted from
    --   applying immutable DB blocks, starting from the successor of 'b'.
    --
    ...
    -- the anchor.
    let dbPrunedToAnchor = LedgerDB.ledgerDbPrune (SecurityParam 0) db
    (varDB, varPrevApplied) <-
      (,) <$> newTVarIO dbPrunedToAnchor <*> newTVarIO Set.empty
    return (
        LgrDB {
            varDB          = varDB
