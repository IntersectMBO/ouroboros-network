...
toPRTCLState :: TPraosChainState c -> STS.State (STS.PRTCL c)
toPRTCLState cs
...
lastSlot :: TPraosChainState c -> WithOrigin SlotNo
lastSlot cs
  | Map.null (historicalStates cs) = error "Empty chain state"
  | otherwise = fst . Map.findMax $ historicalStates cs
...
appendState ss st = let
    STS.PrtclState _ _ slot _ _ _ _ = ss
  in st
    { historicalStates = Map.insert (At slot) ss (historicalStates st) }
