...
deriving instance (IsMapKind mk, LedgerSupportsProtocol blk) => Show (ExtLedgerState blk mk)
...
instance (IsMapKind mk, LedgerSupportsProtocol blk) => NoThunks (ExtLedgerState blk mk) where
