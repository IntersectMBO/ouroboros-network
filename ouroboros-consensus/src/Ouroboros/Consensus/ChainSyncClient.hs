...
data ChainSyncClientException hdr =
      ...
      --
      -- TODO: That String should go (the two use sites should be different
      -- constructors).
      TooFarInTheFuture String SlotNo SlotNo
  ...
  where
    ...
    rollForward varCandidate hdr theirHead = atomically $ do
      ...
      when (unSlotNo theirSlot > unSlotNo currentSlot + maxSkew) $
        disconnect $ TooFarInTheFuture "exceed clock skew" theirSlot currentSlot

      CandidateState {..} <- readTVar varCandidate

      ledgerView <-
        case anachronisticProtocolLedgerView cfg curLedger theirSlot of
          Nothing ->
            disconnect $ TooFarInTheFuture "anachronisticProtocolLedgerView returned Nothing" theirSlot currentSlot
          Just view ->
            case atSlot theirSlot view of
              Nothing -> disconnect $ TooFarInTheFuture ("atSlot returned Nothing; bounds: " ++ show (sbLower view, sbUpper view)) theirSlot currentSlot
              Just lv -> return lv
    ...
    rollBackward varCandidate intersection theirHead = atomically $ do
      CandidateState {..} <- readTVar varCandidate
      let _theirSlot = AF.pointSlot theirHead
