...
summaryToEpochInfo :: forall m xs. (MonadSTM m, MonadThrow (STM m))
                   => RelativeTime -> STM m (Summary xs) -> m (EpochInfo (STM m))
summaryToEpochInfo sysRelTime =
    fmap go . runWithCachedSummary sysRelTime
  where
...
snapshotEpochInfo :: forall xs. RelativeTime -> Summary xs -> EpochInfo Identity
snapshotEpochInfo sysRelTime summary = EpochInfo {
  ...
  where
    ...
    runQueryPure' = Identity . flip (runQueryPure sysRelTime) summary
