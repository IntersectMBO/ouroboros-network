...
module Ouroboros.Consensus.Node.ProtocolInfo.Mock.TPraos (
    protocolInfoTPraos
  ) where
...
protocolInfoTPraos :: NumCoreNodes
                   -> CoreNodeId
                   -> TPraosParams
                   -> ProtocolInfo (SimpleTPraosBlock SimpleMockCrypto
                                                     TPraosMockCrypto)
protocolInfoTPraos (NumCoreNodes numCoreNodes) (CoreNodeId nid) params =
    ProtocolInfo {
        pInfoConfig = EncNodeConfig {
            encNodeConfigP = TPraosNodeConfig {
                praosParams        = params
