...
prop_all_common_prefix l (c:cs) = conjoin [prop_common_prefix l n c d | (d, n) <- zip cs [1..] ]

prop_common_prefix :: forall b. (HasHeader b, Condense b, Eq b)
                   => Word64 -> Word64 -> Chain b -> Chain b -> Property
prop_common_prefix l n x y = go 0 n x y .&&. go n 0 y x
  where
    go nc nd c d =
        let (l', _) = findPrefix c d
            e        = "after dropping "
                 <> show l'
                 <> " block(s) from the final chain of node " <> show nc
                 <> ",\nthe resulting chain is a prefix of "
                 <> "the final chain of node " <> show nd
                 <> ",\nbut only "
                 <> show l
    ...
    _showChain :: Chain b -> String
    _showChain c = condense c
                  <> "\n(length "
                  <> show (Chain.length c)
