...
data instance LedgerState BlockA = LgrA {
      ...
      -- | The slot number of the 'InitiateAtoB' transaction.
...
instance SingleEraBlock BlockA where
  ...
  singleEraTransition (ei, cfg) st = do
      ...
      -- least @k@ blocks have come after). It must not report it sooner than
      -- that because the consensus layer requires that conversions about time
      -- (when successful) must not be subject to rollback.
      ...
      -- All @ei@ invocations below are at @sno@ or earlier,
      -- so we should see no 'PastHorizonException'.

      let
          -- 'EpochNo' of the block containing the 'InitiateAtoB' transaction
          txEpoch = runEI epochInfoEpoch confirmedInSlot

          -- First slot and epoch size of the epoch containing that tx
          firstSlotTxEpoch = runEI epochInfoFirst txEpoch
          epochSizeTxEpoch = runEI epochInfoSize  txEpoch

          -- Consensus insists that as long as the transition to the next era is
          -- not yet known (ie not yet determined by an immutable block), there
          -- is a safe zone that extends past the tip of the ledger in which we
          -- guarantee the next era will not begin. This means that we must have
          -- an additional @safeFromTipA k@ blocks /after/ reporting the
          -- transition and /before/ the start of the next era.
          --
          -- Thus, we schedule the next era to begin with the first upcoming
          -- epoch that starts /after/ we're guaranteed to see both the
          -- aforementioned @k@ additional blocks and also a further
          -- @safeFromTipA k@ slots after the last of those.
          ...
          lastEpochThisEra =
              History.addEpochs
                (History.countSlots
                   firstPossibleLastSlotThisEra firstSlotTxEpoch
                 `div` unEpochSize epochSizeTxEpoch)
                txEpoch
      ...
      txEpoch `seq` Nothing -- return firstEpochNextEra
    where
      k = lcfgA_k cfg
