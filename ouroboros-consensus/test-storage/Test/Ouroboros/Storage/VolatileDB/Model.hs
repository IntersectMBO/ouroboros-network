...
getBlockToPredecessor DBModel { fileIndex, codecConfig } =
    foldMap (fileBlockToPredecessor codecConfig) fileIndex
...
fileBlockToPredecessor ::
  ...
  -> BlocksInFile blk
  -> Map (HeaderHash blk) (ChainHash blk)
fileBlockToPredecessor ccfg (BlocksInFile blks) = Map.fromList
    [ (blockHash blk, blockPrevHash ccfg blk)
    | blk <- blks
    ]
...
getBlockInfoModel dbm@DBModel { codecConfig } = whenOpen dbm $ \hash ->
    extractBlockInfo codecConfig <$> Map.lookup hash (blockIndex dbm)
