...
* Added two new fields to `BlockFetchConsensusInterface`:
  `readChainSelStarvation` indicates to BlockFetch if block validation is
  waiting for more blocks to arrive. `demoteChainSyncJumpingDynamo` indicates to
  the consensus layer that BlockFetch stopped downloading blocks from a given
  peer. These fields are only used by the Genesis fetch mode at the moment.
