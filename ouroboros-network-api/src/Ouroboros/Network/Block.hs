...
unwrapCBORinCBOR :: (forall s. Maybe Lazy.ByteString -> Decoder s a)
...
fromSerialised :: (forall s. Maybe Lazy.ByteString -> Decoder s a)
               -> Serialised a -> (forall s. Decoder s a)
fromSerialised dec (Serialised payload) =
    case Read.deserialiseFromBytes (dec (Just payload)) payload of
