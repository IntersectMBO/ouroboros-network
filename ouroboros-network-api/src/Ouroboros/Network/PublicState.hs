...
import Ouroboros.Network.PeerSelection.PeerSharing.Codec (RemoteAddressCBOR (..))
...
encodeNetworkState
    :: Serialise (RemoteAddressCBOR peeraddr)
...
encodeNetworkState
  ...
  =  encodeListLen 9
  -- outbound state
  <> encode (RemoteAddressCBOR `map` Set.toList coldPeers)
  <> encode (RemoteAddressCBOR `map` Set.toList warmPeers)
  <> encode (RemoteAddressCBOR `map` Set.toList hotPeers)
  ...
  <> Map.foldMapWithKey (\connId st -> encodeListLen 2
                                    ...
                                    <> encodeConnState st)
                        connectionMap
  <> encode (RemoteAddressCBOR `map` Set.toList registeredOutboundConnections)

  where
    toPair :: ConnectionId addr -> (RemoteAddressCBOR addr, RemoteAddressCBOR addr)
    toPair ConnectionId { remoteAddress, localAddress } =
      (RemoteAddressCBOR remoteAddress, RemoteAddressCBOR localAddress)
...
decodeNetworkState
  :: (Serialise (RemoteAddressCBOR peeraddr), Ord peeraddr)
  => Decoder s (NetworkState peeraddr)
decodeNetworkState = do
    _ <- decodeListLen
    -- outbound state
    coldPeers <- Set.fromList . map getRemoteAddressCBOR <$> decode
    warmPeers <- Set.fromList . map getRemoteAddressCBOR <$> decode
    hotPeers <- Set.fromList . map getRemoteAddressCBOR <$> decode
    ...
    connectionMap
      ...
      return (connId, st)
    registeredOutboundConnections
      <- Set.fromList . map getRemoteAddressCBOR <$> decode
  ...
  where
    fromPair :: (RemoteAddressCBOR addr, RemoteAddressCBOR addr) -> ConnectionId addr
    fromPair (RemoteAddressCBOR remoteAddress, RemoteAddressCBOR localAddress) =
