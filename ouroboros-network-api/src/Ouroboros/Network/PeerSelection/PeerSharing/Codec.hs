...
--
newtype RemoteAddressCBOR addr = RemoteAddressCBOR { getRemoteAddressCBOR :: addr }

-- | This instance is used by `LocalStateQuery` mini-protocol codec in
-- `ouroboros-consensus-diffusion`.
--
instance Serialise (RemoteAddressCBOR SockAddr) where
  encode = encodeRemoteAddress . getRemoteAddressCBOR
  decode = RemoteAddressCBOR <$> decodeRemoteAddress
