...
windowMeasure :: FT.Measured v a
              => Window v a
              -> v
windowMeasure = wmV . FT.measure . windowTree
...
size :: FT.Measured v a
     => Window v a
     -> Int
size = wmCount . FT.measure . windowTree
{-# INLINE size #-}
...
evictOldest win@Window { windowTree } =
  case FT.viewr windowTree of
    FT.EmptyR      -> win
    prefix FT.:> _ ->
      let !_ = FT.measure prefix
       in win { windowTree = prefix }
...
evictOldestN n win
    ...
    | otherwise  = takeUntil (\v -> wmCount v > keep) win
  where
    total = size win
    keep  = total - n
...
trimByMeasure :: FT.Measured v a
              ...
              -> Window v a
              -> Window v a
trimByMeasure p = takeUntil (p . wmV)
...
resize n win
  ...
  | size win > n = let win' = win { windowMaxSize = n }
                    in takeUntil (\v -> wmCount v > n) win'
...
fromFoldable :: forall t v a b. (FT.Measured v b, Foldable t, Coercible a b)
             ...
             -> t a
             -> Window v b
fromFoldable n as = takeUntil (\v -> wmCount v > n) win
  where
    win = Window { windowMaxSize = n, windowTree }
    windowTree = foldl' (\ft a -> Sample (coerce a) FT.<| ft) FT.empty as
