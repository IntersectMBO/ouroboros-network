...
instance LedgerSupportsPeerSelection (ShelleyBlock era) where
  getPeers ShelleyLedgerState { shelleyLedgerState } = catMaybes
    ...
    where
      ...
      -- relayToRelayAddress (SL.SingleHostAddr (SJust (Port port)) SNothing (SJust ipv6)) = -- TODO Connection manager + p2p governor lacks IPv6 support
      ...
      pparamsRelayAddresses ::
           SL.PoolParams era
        -> Maybe (NonEmpty RelayAddress)
      pparamsRelayAddresses =
          NE.nonEmpty . catMaybes . map relayToRelayAddress . toList . SL._poolRelays

      -- | Combine the stake pools registered in the future and the current pool
      -- parameters, and remove duplicates.
      poolRelayAddresses ::
           Map (SL.KeyHash 'SL.StakePool (EraCrypto era)) (NonEmpty RelayAddress)
      poolRelayAddresses =
          Map.unionWith
            (\futureRelays currentRelays -> NE.nub (futureRelays <> currentRelays))
            (Map.mapMaybe pparamsRelayAddresses futurePoolParams)
            (Map.mapMaybe pparamsRelayAddresses poolParams)
