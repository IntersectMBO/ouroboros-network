...
data ShelleyLedgerConfig era = ShelleyLedgerConfig {
      ...
      -- 'ShelleyBlock' -- such a 'ShelleyLedgerConfig' will have been
      -- /completed/ by the HFC to contain the most information 'EpochInfo'
      -- possible base on the past hard forks (and possibly also the next hard
      -- fork, if its occurrence is already known).
