...
--   TPraos header.
instance (CanMock era (Praos crypto), crypto ~ EraCrypto era)
...
instance (CanMock era (Praos crypto), crypto ~ EraCrypto era)
  ...
  arbitrary = ShelleyHeader <$> arbitrary <*> arbitrary

instance SL.Mock c => Arbitrary (ShelleyHash c) where
  arbitrary = ShelleyHash <$> arbitrary

instance CanMock era proto => Arbitrary (GenTx (ShelleyBlock proto era)) where
  arbitrary = mkShelleyTx <$> arbitrary

instance CanMock era proto => Arbitrary (GenTxId (ShelleyBlock proto era)) where
  arbitrary = ShelleyTxId <$> arbitrary

instance CanMock era proto => Arbitrary (SomeSecond BlockQuery (ShelleyBlock proto era)) where
  arbitrary = oneof
...
instance CanMock era proto => Arbitrary (SomeResult (ShelleyBlock proto era)) where
  arbitrary = oneof
...
instance CanMock era proto => Arbitrary (Point (ShelleyBlock proto era)) where
...
instance CanMock era proto=> Arbitrary (ShelleyTip proto era) where
...
instance CanMock era proto => Arbitrary (LedgerState (ShelleyBlock proto era)) where
  arbitrary = ShelleyLedgerState
    <$> arbitrary
    <*> arbitrary
    <*> arbitrary

instance CanMock era proto => Arbitrary (AnnTip (ShelleyBlock proto era)) where
  arbitrary = AnnTip
    <$> arbitrary
    <*> (BlockNo <$> arbitrary)
    <*> arbitrary
