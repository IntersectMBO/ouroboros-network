...
instance CanMock era => Arbitrary (SomeQuery (BlockQuery (ShelleyBlock era))) where
  arbitrary = oneof
...
instance CanMock era
      => Arbitrary (WithVersion ShelleyNodeToClientVersion (SomeQuery (BlockQuery (ShelleyBlock era)))) where
  arbitrary = do
