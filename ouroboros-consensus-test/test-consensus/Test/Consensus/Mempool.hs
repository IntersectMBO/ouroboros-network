...
prop_Mempool_semigroup_addTxs setup =
    withTestMempool (testSetup setup) $ \TestMempool{mempool = mempool1} -> do
        ...
        return $
            withTestMempool (testSetup setup) $ \TestMempool{mempool = mempool2} -> do
...
prop_Mempool_semigroup_removeTxs (TestSetupWithTxsInMempool testSetup txsToRemove) =
    withTestMempool testSetup $ \TestMempool{mempool = mempool1} -> do
        ...
        return $
            withTestMempool testSetup $ \TestMempool{mempool = mempool2} -> do
...
prop_Mempool_Capacity (MempoolCapTestSetup testSetupWithTxs) =
    withTestMempool testSetup $ \TestMempool{mempool} -> do
        ...
        curSize <-
            msNumBytes . snapshotMempoolSize
                <$> atomically (getSnapshot mempool)
        ...
        go curSize processed = \case
            ...
            (tx, valid) : txsToAdd'
                | let curSize' = curSize + txSize tx
                  , curSize' <= capacity ->
                    go
                        (if valid then curSize' else curSize)
                        ((tx, valid) : processed)
                        txsToAdd'
                | otherwise ->
