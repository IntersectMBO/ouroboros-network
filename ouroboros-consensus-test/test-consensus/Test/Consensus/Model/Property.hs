...
module Test.Consensus.Model.Property (
...
import Test.Consensus.Model.Mempool (
    Action (..),
    Mempool (..),
    RunMonad (..),
 )
...
prop_MemPool :: Actions Mempool -> Property
prop_MemPool actions = property $
    runIOSimProp $ do
        _ <- runActions actions
        assert True

accept_all_non_conflicting_txs :: DL Mempool ()
...
runIOSimProp p = do
    ...
    let tr = runSimTrace $ (runMonad $ eval $ monadic' p)
