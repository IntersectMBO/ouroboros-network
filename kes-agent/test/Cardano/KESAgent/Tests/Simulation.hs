      ...
      withAddress $ \serviceAddress -> do
        ...
        output <-
          race
            ...
            ( agent tracer
                `concurrently_` nodes tracer
                `concurrently_` controlClients tracer
            )
            -- ...until we get a property, or the thread delay kicks in.
            (threadDelay 1000000 `race` watch doneVar)

        prop <-
          fromMaybe (counterexample "Interrupted" $ property False)
              ...
              case Map.lookup (creID ev) m of
                ...
                Just v -> do
                  ...
                  when
                    (creCountBefore ev /= v)
                    ( error $
                        "CRef count mismatch before acquire; expected "
                          ++ show (creCountBefore ev)
                          ++ ", but found "
                          ++ show v
                    )
                  when
                    (creCountAfter ev /= v')
                    ( error $
                        "CRef count mismatch after acquire; expected "
                          ++ show (creCountAfter ev)
                          ++ ", but found "
                          ++ show v'
                    )
              ...
              case Map.lookup (creID ev) m of
                ...
                Just v -> do
                  ...
                  when
                    (creCountBefore ev /= v)
                    ( error $
                        "CRef count mismatch before release; expected "
                          ++ show (creCountBefore ev)
                          ++ ", but found "
                          ++ show v
                    )
                  when
                    (creCountAfter ev /= v')
                    ( error $
                        "CRef count mismatch after release; expected "
                          ++ show (creCountAfter ev)
                          ++ ", but found "
                          ++ show v'
                    )
