...
tests =
  testGroup
    ...
    [ testGroup
        "fixtures"
        [ testDeserializeFixture
            "Cold VerKey"
            "fixtures/cold.vkey"
            (Proxy @(ColdVerKey (DSIGN StandardCrypto)))
        , testDeserializeFixture
            "Cold SignKey"
            "fixtures/cold.skey"
            (Proxy @(ColdSignKey (DSIGN StandardCrypto)))
        , testDeserializeFixture "KES VerKey" "fixtures/kes.vkey" (Proxy @(KESVerKey (KES StandardCrypto)))
        , testDeserializeFixture
            "OpCert Counter"
            "fixtures/opcert.counter"
            (Proxy @(OpCertCounter StandardCrypto))
...
mkVerKeyKES ::
  PinnedSizedBytes (SeedSizeKES (KES StandardCrypto)) -> IO (KESVerKey (KES StandardCrypto))
mkVerKeyKES seed =
  KESVerKey <$> do
...
mkColdSignKey ::
  PinnedSizedBytes (SeedSizeDSIGN (DSIGN StandardCrypto)) -> IO (ColdSignKey (DSIGN StandardCrypto))
mkColdSignKey seed = return . ColdSignKey $ genKeyDSIGN (mkSeedFromBytes . psbToByteString $ seed)

mkColdVerKey ::
  PinnedSizedBytes (SeedSizeDSIGN (DSIGN StandardCrypto)) -> IO (ColdVerKey (DSIGN StandardCrypto))
mkColdVerKey seed = ColdVerKey . deriveVerKeyDSIGN . coldSignKey <$> mkColdSignKey seed

mkOpCertCounter ::
  (PinnedSizedBytes (SeedSizeDSIGN (DSIGN StandardCrypto)), Word64) ->
  IO (OpCertCounter StandardCrypto)
