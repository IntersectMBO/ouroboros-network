...
instance HasInfo (DirectCodec m) UTCTime where
  info codec _ = aliasField "POSIXSeconds"
...
instance ( Serializable (DirectCodec m) a
         , Serializable (DirectCodec m) Bool
         , Monad m
         )
    => Serializable (DirectCodec m) (Maybe a) where
...
instance ( SodiumHashAlgorithm h
         , HashAlgorithm h
         ) => HasInfo (DirectCodec m) (Hash h a) where
    info _ _ =
...
instance ( MonadST m
         ...
         ) => Serializable (DirectCodec m) (VerKeyDSIGN dsign) where
    encode codec val =
...
instance ( DSIGNAlgorithm dsign
         ) => HasInfo (DirectCodec m) (SignKeyDSIGN dsign) where
   info _ _ =
     aliasField
...
instance ( MonadST m
         ...
         , MonadThrow m
         , DSIGNAlgorithm dsign
         ) => Serializable (DirectCodec m) (SignKeyDSIGN dsign) where
    encode codec val =
...
instance ( Monad m
         ...
         ) => Serializable (DirectCodec m) (SignedDSIGN d a) where
...
instance ( MonadSTM m
         ...
         ) => Serializable (DirectCodec m) (SignKeyKES kes) where
    encode codec val = do
      s <- ask
      lift $ directSerialise
        (\buf bufSize -> do
...
instance ( MonadST m
         ...
         ) => Serializable (DirectCodec m) (VerKeyKES kes) where
    encode codec val =
...
instance ( KnownNat t
         ...
         ) => HasInfo (DirectCodec m) (VerKeyKES (MockKES t)) where
    info codec _ =
      aliasField
...
instance ( KnownNat t
         , KESAlgorithm (MockKES t)
         ) => HasInfo (DirectCodec m) (SignKeyKES (MockKES t)) where
   info codec _ =
     compoundField
...
instance ( DSIGNAlgorithm dsign
         ...
         ) => HasInfo (DirectCodec m) (VerKeyKES (SingleKES dsign)) where
    info codec _ =
      aliasField
...
instance ( DSIGNAlgorithm dsign
         ...
         ) => HasInfo (DirectCodec m) (SignKeyKES (SingleKES dsign)) where
   info codec _ =
     aliasField
...
instance ( HashAlgorithm h
         ...
         ) => HasInfo (DirectCodec m) (VerKeyKES (SumKES h kes)) where
    info codec _ =
      aliasField
...
instance ( SodiumHashAlgorithm h
         ...
         ) => HasInfo (DirectCodec m) (SignKeyKES (SumKES h kes)) where
   info codec _ =
     compoundField
...
instance ( DSIGNAlgorithm dsign
         ...
         ) => HasInfo (DirectCodec m) (VerKeyKES (CompactSingleKES dsign)) where
    info codec _ =
      aliasField
...
instance ( DSIGNAlgorithm dsign
         , KESAlgorithm (CompactSingleKES dsign)
         , HasInfo (DirectCodec m) (SignKeyDSIGN dsign)
         ) => HasInfo (DirectCodec m) (SignKeyKES (CompactSingleKES dsign)) where
   info codec _ =
     aliasField
...
instance ( HashAlgorithm h
         ...
         ) => HasInfo (DirectCodec m) (VerKeyKES (CompactSumKES h kes)) where
    info codec _ =
      aliasField
...
instance ( SodiumHashAlgorithm h
         ...
         , forall a. HasInfo (DirectCodec m) (Hash h a)
         ) => HasInfo (DirectCodec m) (SignKeyKES (CompactSumKES h kes)) where
   info codec _ =
     compoundField
...
instance (KESAlgorithm kes, HasInfo (DirectCodec m) (SignKeyKES kes))
          => HasInfo (DirectCodec m) (SignKeyWithPeriodKES kes) where
  info codec _ =
    compoundField
...
instance ( MonadThrow m
         ...
         => Serializable (DirectCodec m) (SignKeyWithPeriodKES kes) where
  encode s skp = do
...
instance (HasInfo (DirectCodec m_axB5) (VerKeyKES (KES c_ieDN)),
          HasInfo (DirectCodec m_axB5) (SignedDSIGN (DSIGN c_ieDN) (OCertSignable c_ieDN))) =>
         HasInfo (DirectCodec m_axB5) (OCert c_ieDN) where
  info codec _
    = (compoundField "OCert")
        [("ocertVkHot",
...
instance (Monad (MonadEncode (DirectCodec m_axB5)),
         ...
         Serializable (DirectCodec m_axB5) (OCert c_ieDN) where
  encode p item
    = sequence_
        [(encode p) (ocertVkHot item),
         (encode p) (ocertN item),
         (encode p) (ocertKESPeriod item), (encode p) (ocertSigma item)]
  decode p
...
instance ( HasInfo (DirectCodec m) (SignKeyKES (KES c))
         , HasInfo (DirectCodec m) (OCert c)
         , Typeable c
         , Crypto c
         , KESAlgorithm (KES c)
         )
         => HasInfo (DirectCodec m) (Bundle m c) where
  info codec _ =
    compoundField
...
instance ( MonadThrow m
         ...
         => Serializable (DirectCodec m) (Bundle m c) where
...
instance ( MonadST m
         ...
         , MonadThrow m
         , DSIGNAlgorithm dsign
         ) => Serializable (DirectCodec m) (SigDSIGN dsign) where
    encode codec val =
...
deriving via (ViaEnum RecvResult)
  instance HasInfo (DirectCodec m) RecvResult

instance
  ( MonadThrow m
  , MonadST m
  ) => Serializable (DirectCodec m) RecvResult where
