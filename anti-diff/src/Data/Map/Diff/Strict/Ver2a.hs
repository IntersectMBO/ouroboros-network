...
module Data.Map.Diff.Strict.Ver2a (
    Diff (..)
  , DiffEntry (..)
  , DiffHistory (..)
    -- * Construction
...
-- /latest/ change.
data DiffHistory v =
    DiffHistory !(Seq (DiffEntry v))
  | EmptyDiffHistory
  | AntiDiffHistory !(Seq (DiffEntry v))
  deriving stock (Generic, Show, Eq, Functor)
  deriving anyclass (NoThunks)
...
instance Eq v => Semigroup (DiffHistory v) where
  h1 <> h2 = case (h1, h2) of
      (DiffHistory s1, DiffHistory s2)         -> DiffHistory $ s1 <> s2
      (AntiDiffHistory s1, DiffHistory s2)     ->
        (Seq.reverse . fmap invertDiffEntry $ s1) `mappend'` s2
      (DiffHistory s1, AntiDiffHistory s2)     ->
        s1 `mappend'` (Seq.reverse . fmap invertDiffEntry $ s2)
      (AntiDiffHistory s1, AntiDiffHistory s2) -> AntiDiffHistory $ s1 <> s2
    where
      mappend' Empty Empty                   = EmptyDiffHistory
      ...
      mappend' xs0@(xs :|> x) ys0@(y :<| ys)
        ...
        | otherwise                          = DiffHistory $ xs0 Seq.>< ys0
      mappend' xs0 ys0                       = DiffHistory $ xs0 Seq.>< ys0

data LMR a b c = L {getA :: a } | M b b | R {getC :: c} | E

instance Eq v => Monoid (DiffHistory v) where
  mempty = DiffHistory mempty

instance Eq v => Group (DiffHistory v) where
  invert (DiffHistory s)     = AntiDiffHistory s
  invert EmptyDiffHistory    = EmptyDiffHistory
  invert (AntiDiffHistory s) = DiffHistory s
...
isNonEmptyHistory EmptyDiffHistory = Nothing
isNonEmptyHistory h                = Just h
