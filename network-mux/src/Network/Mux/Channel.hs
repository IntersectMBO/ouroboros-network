...
data Channel m a = Channel {
    ...
    -- channel).
    --
    -- The map inside 'Reception' records the time at which each chunk of the
    -- input bytes data was read from the bearer. The key is the index of the
    -- first byte of the arrival. Thus the first entry always has key @0@.
    -- Some 'Channel's are more abstract than to have a 'Bearer'; these times
    -- are a best effort in those cases and so may be very coarse, but they're
    -- never missing.
    --
...
handlesAsChannel hndRead hndWrite =
  ...
  where
    ...
    recv = do
      ...
      if eof
        then return Nothing
        else Just . MkReception (IntMap.singleton 0 tm) . LBS.fromStrict
               <$> BS.hGetSome hndRead LBS.smallChunkSize
  ...
  where
    buffersAsChannel bufferRead bufferWrite =
      ...
      where
        ...
        recv   = do
            tm <- getMonotonicTime
            Just . MkReception (IntMap.singleton 0 tm) . LBS.fromStrict
