...
--
-- This exception should not be handled by a simulation, it is a proof of a half
...
newAttenuatedChannel tr Attenuation { aReadAttenuation
  ...
  where
    acRead :: m BL.ByteString
    acRead = do
      msg <- atomically $ readQueueChannelSTM qc
      t <- getMonotonicTime
      case msg of
        -- match the 'Bearer.Snocket' behavior and throw 'MuxError'
    ...
    acClose = do
      ...
      traceWith tr (AttenuatedChannelClosing sent)
      ...
      traceWith tr (AttenuatedChannelClosed (isJust res))
      case res of
        Just _  -> return ()
...
data AttenuatedChannelTrace =
    AttenuatedChannelClosing Bool
  | AttenuatedChannelClosed  Bool
  deriving Show
