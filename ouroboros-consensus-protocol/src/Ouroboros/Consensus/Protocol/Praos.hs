...
data PraosToSign c = PraosToSign
  { -- | Verification key for the issuer of this block.
    ...
    praosToSignVrfRes   :: SL.CertifiedVRF c UnifiedVRF,
...
newtype PraosIsLeader c = PraosIsLeader
  { praosIsLeaderVrfRes :: SL.CertifiedVRF c UnifiedVRF
  }
  deriving (Generic)
...
instance PraosCrypto c => Serialise (PraosState c) where
  ...
  decode =
    decodeVersion
      [(serialisationFormatVersion1, Decode decodePraosState1)]
    where
      decodePraosState1 = do
...
data PraosValidationErr c
  ...
  | VRFKeyBadNonce
      ...
      !(VRF.CertifiedVRF (VRF c) UnifiedVRF) -- VRF calculated nonce value
  | VRFLeaderValueTooBig (VRF.OutputVRF (VRF c)) Rational ActiveSlotCoeff
  | KESBeforeStartOCERT
...
instance PraosCrypto c => ConsensusProtocol (Praos c) where
  ...
  checkIsLeader
    ...
    slot
    cs =
      if meetsLeaderThreshold cfg lv (SL.coerceKeyRole vkhCold) y
        then
          Just
            PraosIsLeader
      ...
      where
        ...
        rho' = mkUnifiedVRF slot eta0

        rho = VRF.evalCertified () rho' praosCanBeLeaderSignKeyVRF
        y = vrfLeaderCheckValue (Proxy @c) $ certifiedOutput rho
  ...
  reupdateChainDepState
    ...
    tcs =
      ...
      where
        ...
        eta =
          mkNonceFromOutputVRF . vrfNonceValue (Proxy @c)
            . VRF.certifiedOutput
            $ Views.hvVrfRes b
...
meetsLeaderThreshold
  ...
  unifiedVRF =
    checkLeaderValue
      (vrfLeaderCheckValue (Proxy @c) unifiedVRF)
      r
...
validateVRFSignature eta0 (Views.lvPoolDistr -> SL.PoolDistr pd) f b = do
  ...
  case sigma' of
    ...
    Just (IndividualPoolStake sigma vrfHK) -> do
      ...
      VRF.verifyCertified
        ()
        vrfK
        (mkUnifiedVRF slot eta0)
        vrfCert
        ?! VRFKeyBadNonce slot eta0 vrfCert
      checkLeaderValue vrfLeaderVal sigma f
  ...
  where
    ...
    unifiedVRF = VRF.certifiedOutput vrfCert
    vrfLeaderVal = vrfLeaderCheckValue (Proxy @c) unifiedVRF
