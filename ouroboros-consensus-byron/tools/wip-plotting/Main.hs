    ...
    ss = Nothing : map Just (Set.toList $ go1 0 Set.empty s0)
      where
        go1 n acc x
            ...
            | otherwise        = go2 n (Set.insert x acc) x
        go2 n acc x = case x of
          Seq.Empty  -> Set.singleton Seq.Empty
          _ Seq.:<| y -> (f EmptySlot . f ActiveSlot) acc
            where
              f :: SlotStatus -> Set.Set (Seq.Seq SlotStatus) -> Set.Set (Seq.Seq SlotStatus)
              f s acc' = go1 (n+1) acc' (y Seq.:|> s)
