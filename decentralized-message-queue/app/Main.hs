                           ...
                           [ Mx.MiniProtocol
                               ...
                               , miniProtocolRun =
                                   Mx.InitiatorProtocolOnly
                                     ...
                                     . const
                                     $ (nullTracer, cStateQueryCodec, StateIdle, localStateQueryClientPeer $ client (WithEventType "LocalStateQuery" >$< tracer) (poolIds nodeKernel))
                               }
                           ]
...
client tracer PoolIds { getPoolIds } = go
  where
    ...
    acquiring = ClientStAcquiring {
       recvMsgAcquired =
         pure . SendMsgQuery (BlockQuery . QueryIfCurrentBabbage $ GetStakePools) $ ClientStQuerying \cardanoQueryResult-> do
      ...
      , recvMsgFailure = \failure -> do
          traceWith tracer $ "recvMsgFailure: " <> show failure
          threadDelay 86400
          runLocalStateQueryClient go
      }
