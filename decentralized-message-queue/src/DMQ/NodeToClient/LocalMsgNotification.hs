        ...
        msgRequestHandler blocking = do
          ...
          case blocking of
            SingBlocking
              ...
              | otherwise -> do
                  mResult <- timeoutWithControlMessage controlMessageSTM do
                  ...
                  case mResult of
                    Nothing -> do
                      traceWith tracer TraceMsgNotificationServerDone
                      ServerDone <$> mdone
                    Just (lastIdx', hasMore', msgs) -> do
...
data TraceMessageNotificationServer msg =
  ...
  | TraceMsgNotificationServerDone
    -- ^ server terminates
