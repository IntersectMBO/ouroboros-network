...
sequenceWC update constr@WithConstr { mkKey, ctx } = do
...
shrinkWithConstr update shrinker constr =
       unsafePerformIO (sequenceWC update $ shrinker <$> constr)
    ++ shrinkWithConstrCtx constr


runWithConstr :: WithConstr ctx key a
              -> IO a
...
instance ( kesCrypto ~ KES crypto
      ...
      => Arbitrary (WithConstrVerKeyKES size kesCrypto (SigRaw crypto)) where
  arbitrary = do
    sigRawId <- arbitrary
  ...
  shrink = shrinkWithConstr updateFn shrinkSigRawFn
    where
      updateFn :: SigRaw crypto -> VerKeyKES kesCrypto -> SigRaw crypto
      updateFn
        sig@SigRaw {sigRawOpCertificate = SigOpCertificate opCert}
        ocertVkHot
        =
        sig { sigRawOpCertificate = SigOpCertificate opCert { ocertVkHot } } 
...
shrinkSigFn SigWithBytes {sigRaw} = mkSig <$> shrinkSigRawFn sigRaw
