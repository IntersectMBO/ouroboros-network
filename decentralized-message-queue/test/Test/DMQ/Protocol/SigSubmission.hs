...
import Cardano.KESAgent.Protocols.StandardCrypto (StandardCrypto, MockCrypto)
    ...
    [ testGroup "SigSubmission"
      [ testGroup "Codec"
        [ testGroup "MockCrypto"
          [ testProperty "bytes"               prop_bytes_mockcrypto
        ...
        , testGroup "StandardCrypto"
          [ testProperty "bytes"               prop_bytes_standardcrypto
...
-- | Check that raw bytes and signed bytes match with the `Sig` data structure.
--
prop_bytes
  :: ( Crypto crypto
     , Typeable crypto
  ...
  -> Property
prop_bytes constr = ioProperty $ do
    sig <- runWithConstr constr
    let SigWithBytes { sigRawBytes,
                       sigRawWithSignedBytes =
                         SigRawWithSignedBytes {
                           sigRaw,
                           sigRawSignedBytes
        ...
        bytes       = CBOR.toLazyByteString (encodeSigRaw  sigRaw)
        signedBytes = CBOR.toLazyByteString (encodeSigRaw' sigRaw)
    return $ counterexample "serialised bytes do not match"
             (bytes === sigRawBytes)
        .&&. counterexample "signed bytes do not match"
             (signedBytes === sigRawSignedBytes)

prop_bytes_mockcrypto
  :: Blind (WithConstrKES (SeedSizeKES (KES MockCrypto)) (KES MockCrypto) (Sig MockCrypto))
  -> Property
prop_bytes_mockcrypto = prop_bytes . getBlind

prop_bytes_standardcrypto
  :: Blind (WithConstrKES (SeedSizeKES (KES StandardCrypto)) (KES StandardCrypto) (Sig StandardCrypto))
  -> Property
prop_bytes_standardcrypto = prop_bytes . getBlind
