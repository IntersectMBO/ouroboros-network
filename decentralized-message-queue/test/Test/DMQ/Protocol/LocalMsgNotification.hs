...
--
prop_connect :: Fixed Int -> Positive Word16 -> DistinctList Sig -> Bool
prop_connect (Fixed seed) (Positive maxMsgs) (DistinctList msgs) =
  case runSimOrThrow
         (connect
           (localMsgNotificationServerPeer $
            testServer testTracer (mkStdGen seed) maxMsgs msgs)
...
prop_channel :: (MonadAsync m, MonadCatch m, Eq msg, Show msg, ShowProxy msg)
             ...
             -> Codec' m msg
             -> Fixed Int -> Positive Word16 -> DistinctList msg
             -> m Property
prop_channel createChannels codec
             (Fixed seed) (Positive maxMsgs) (DistinctList msgs) =
    (\((), msgs') -> msgs' === msgs) <$>
    ...
    runConnectedPeers
      ...
      codec
      (localMsgNotificationServerPeer $
         testServer (("server",) `contramap` testTracer) (mkStdGen seed) maxMsgs msgs)
      (localMsgNotificationClientPeer $
         testClient (("client",) `contramap` testTracer) maxMsgs)

-- | Run 'prop_channel' in the simulation monad.
--
prop_channel_ST :: Fixed Int -> Positive Word16 -> DistinctList Sig
                -> Property
prop_channel_ST seed maxMsgs msgs =
  runSimOrThrow
    (prop_channel createConnectedChannels codec seed maxMsgs msgs)

-- | Run 'prop_channel' in the IO monad.
--
prop_channel_IO :: Fixed Int -> Positive Word16 -> DistinctList Sig
                -> Property
prop_channel_IO seed maxMsgs msgs =
    ioProperty $
      prop_channel createConnectedBufferedChannelsUnbounded codec seed maxMsgs msgs
...
--
prop_direct :: Fixed Int -> Positive Word16 -> DistinctList Sig -> Bool
prop_direct (Fixed seed) (Positive maxMsgs) (DistinctList sigs) =
    runSimOrThrow
      (direct
        (testServer testTracer (mkStdGen seed) maxMsgs sigs)
  ...
  where
    ...
    go ServerIdle { msgRequestHandler, msgDoneHandler } client = do
      case client of
        ...
        SendMsgRequestBlocking mdone k -> do
          response <- msgRequestHandler SingBlocking
          case response of
            ServerDone a -> (,a) <$> mdone
...
type Codec' m msg = Codec (LocalMsgNotification msg)
...
testServer :: Monad m
           ...
           -> StdGen -> Word16 -> [msg]
           -> TestServer m msg
testServer tracer rnd maxSigs sigs =
    msgNotificationServer
      tracer rnd maxSigs sigs
