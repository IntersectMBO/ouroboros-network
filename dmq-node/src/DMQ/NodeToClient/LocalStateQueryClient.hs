...
module DMQ.NodeToClient.LocalStateQueryClient where
...
-- TODO to stay on the safe side, delay running the main query if just before the epoch boundary
cardanoClient
...
cardanoClient _tracer StakePools { stakePoolsVar } nextEpochVar = LocalStateQueryClient (idle Nothing)
  where
    idle mSystemStart = pure $ SendMsgAcquire ImmutableTip acquire
      where
        ...
        acquire = ClientStAcquiring {
          recvMsgAcquired =
            let snapshotsQry systemStart = pure $
                    SendMsgQuery (BlockQuery . QueryIfCurrentConway $ GetStakeSnapshots Nothing)
                  $ wrappingMismatch (handleStakeSnapshots systemStart)
            in case mSystemStart of
                 Just systemStart -> snapshotsQry systemStart
                 Nothing -> pure $
                   SendMsgQuery GetSystemStart $ ClientStQuerying snapshotsQry
    ...
    getInterpreter systemStart epoch = ClientStQuerying \interpreter -> do
      ...
      case res of
        ...
        Right relativeTime -> do
          ...
          atomically $ writeTVar nextEpochVar (Just nextEpoch)
    ...
    -- should probably get the epoch first, and only get the stake pools if we aren't
    -- immediately before the epoch ends (maybe say 5s) to keep things in sync
    handleStakeSnapshots systemStart StakeSnapshots { ssStakeSnapshots } = do
      atomically $
