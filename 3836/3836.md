...
I've created a new type for `ControlMessage`s that can only represent the two
`ControlMessage`s that `decisionSTM` can yield, and made `decisionSTM` yield
values of that type instead, so the match for the impossible case no longer
exists.
