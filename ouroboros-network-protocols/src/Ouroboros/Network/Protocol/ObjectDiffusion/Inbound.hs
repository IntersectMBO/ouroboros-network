...
import Network.TypedProtocol.Peer hiding (pattern Yield)
import Network.TypedProtocol.Peer.Client (pattern Yield)
...
inboundRun (SendMsgDone kDone) =
      Yield MsgDone $ Effect $ Done ReflNobodyAgency <$> kDone
