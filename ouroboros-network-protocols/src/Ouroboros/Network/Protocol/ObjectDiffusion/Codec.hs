...
-- | Byte Limits.
byteLimitsObjectDiffusion ::
  forall bytes polarity objectId object.
  (bytes -> Word) ->
  ProtocolSizeLimits (ObjectDiffusion polarity objectId object) bytes
byteLimitsObjectDiffusion = ProtocolSizeLimits stateToLimit
 where
  stateToLimit ::
    forall (st :: ObjectDiffusion polarity objectId object).
    ActiveState st =>
    StateToken st ->
    Word
...
-- +---------------------------------+---------------+
timeLimitsObjectDiffusion ::
  forall (polarity :: Polarity) (objectId :: Type) (object :: Type).
  ProtocolTimeLimits (ObjectDiffusion polarity objectId object)
...
codecObjectDiffusion ::
  forall (polarity :: Polarity) (objectId :: Type) (object :: Type) m.
  MonadST m =>
  -- | encode 'objectId'
  ...
  Codec (ObjectDiffusion polarity objectId object) CBOR.DeserialiseFailure m ByteString
...
encodeObjectDiffusion ::
  forall
    ...
    (objectId :: Type)
    (object :: Type)
    (st :: ObjectDiffusion polarity objectId object)
    (st' :: ObjectDiffusion polarity objectId object).
  -- | encode 'objectId'
  (objectId -> CBOR.Encoding) ->
  -- | encode 'object'
  (object -> CBOR.Encoding) ->
  Message (ObjectDiffusion polarity objectId object) st st' ->
  CBOR.Encoding
encodeObjectDiffusion encodeObjectId encodeObject = encode
 where
  encode ::
    forall st0 st1.
    Message (ObjectDiffusion polarity objectId object) st0 st1 ->
    CBOR.Encoding
  encode MsgInit =
...
codecObjectDiffusionId ::
  forall polarity objectId object m.
  Monad m =>
  Codec
    (ObjectDiffusion polarity objectId object)
    CodecFailure
    m
    (AnyMessage (ObjectDiffusion polarity objectId object))
