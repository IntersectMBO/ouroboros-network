...
instance Arbitrary (AnyMessage (ObjectDiffusion ObjectId Object)) where
  arbitrary = oneof
    ...
    , AnyMessage <$> MsgReplyObjectIds <$> NonBlockingReply <$> arbitrary

    , AnyMessage <$> MsgRequestObjects <$> arbitrary

    , AnyMessage <$> MsgReplyObjects <$> arbitrary

    , AnyMessage <$> pure MsgDone
    ]

instance (Eq objectId, Eq object) => Eq (AnyMessage (ObjectDiffusion objectId object)) where
...
codec :: MonadST m
         => Codec (ObjectDiffusion ObjectId Object)
                   DeserialiseFailure
                   m ByteString
